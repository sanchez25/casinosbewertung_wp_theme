<?php
/*Template Name: Casinos*/
?>

<?php get_header()?>

<section class="page-content">
	<div class="head-row">
		<div class="page-block-content">
			<div class="breadcrumps">
				<?php true_breadcrumbs(); ?>
			</div>
			<div class="page-title">
				<h1 class="title-page-h1"><?php the_title(); ?></h1>
			</div>
			<div class="page-main-content">
				<?php
					if ($items = get_posts(array('numberposts' => 50, 'post_type' => 'post'))) {
						echo '<div class="cards">';
							foreach($items as $item) {
								$rating_brend = get_field('rating_brend', $item->ID);
								$text_brend = get_field('text_brend', $item->ID);
								$advant_brend_one = get_field('advant_brend_one', $item->ID);
								$advant_brend_two = get_field('advant_brend_two', $item->ID);
								$advant_brend_three = get_field('advant_brend_three', $item->ID);
								$link_brend = get_field('link_brend', $item->ID);
								$hover_text = get_field('hover_text', $item->ID);

								/* Games */
								$delete_roulette = get_field('delete_roulette', $item->ID);
								$delete_keno = get_field('delete_keno', $item->ID);
								$delete_bingo = get_field('delete_bingo', $item->ID);
								$delete_poker = get_field('delete_poker', $item->ID);
								$delete_slots = get_field('delete_slots', $item->ID);
								$delete_rubbellose = get_field('delete_rubbellose', $item->ID);
								$delete_craps = get_field('delete_craps', $item->ID);
								$delete_baccarat = get_field('delete_baccarat', $item->ID);
								$delete_blackjack = get_field('delete_blackjack', $item->ID);

								$delete_casino = get_field('delete_casino', $item->ID);
								$delete_sports = get_field('delete_sports', $item->ID);
								$delete_live = get_field('delete_live', $item->ID);
								$delete_pok = get_field('delete_pok', $item->ID);
								$delete_bookmaker = get_field('delete_bookmaker', $item->ID);
								$delete_lottery = get_field('delete_lottery', $item->ID);
								$delete_mobile = get_field('delete_mobile', $item->ID);

								echo '<div class="card-page">';
									echo '<div class="card-page-image">';
										echo get_the_post_thumbnail($item->ID);
										echo '<div class="top-rating page">'.$rating_brend.'</div>';
									echo '</div>';
									echo '<div class="card-page-list-block">';
										echo '<div class="card-page-list">';
											echo '<div class="page-list-item-rating">';
												echo '<a class="rating-link-title" href="'.get_permalink($item->ID).'">';
													echo '<h3 class="rating-title">'.$item->post_title.'</h3>';
												echo '</a>';
												echo '<span class="star-top">';
													echo '<svg viewBox="0 0 200 200" class="ikon-rating" id="ico_rep_verygood">';
														echo '<g data-name="Layer 54">';
															echo '<path fill="none" d="M0 0h200v200H0z"></path>';
															echo '<path d="M73.716 63.684l19.192-38.735a8 8 0 0 1 14.347.022l19.072 38.794 42.77 6.282a8 8 0 0 1 4.413 13.652l-31.002 30.126 7.242 42.618a8 8 0 0 1-11.62 8.416l-38.233-20.175-38.294 20.057a8 8 0 0 
																1-11.594-8.451l7.373-42.596-30.909-30.221a8 8 0 0 1 4.455-13.639z">';
															echo '</path>';
														echo '</g>';
													echo '</svg>';
													echo '<svg viewBox="0 0 200 200" class="ikon-rating" id="ico_rep_verygood">';
														echo '<g data-name="Layer 54">';
															echo '<path fill="none" d="M0 0h200v200H0z"></path>';
															echo '<path d="M73.716 63.684l19.192-38.735a8 8 0 0 1 14.347.022l19.072 38.794 42.77 6.282a8 8 0 0 1 4.413 13.652l-31.002 30.126 7.242 42.618a8 8 0 0 1-11.62 8.416l-38.233-20.175-38.294 20.057a8 8 0 0 
																1-11.594-8.451l7.373-42.596-30.909-30.221a8 8 0 0 1 4.455-13.639z">';
															echo '</path>';
														echo '</g>';
													echo '</svg>';
													echo '<svg viewBox="0 0 200 200" class="ikon-rating" id="ico_rep_verygood">';
														echo '<g data-name="Layer 54">';
															echo '<path fill="none" d="M0 0h200v200H0z"></path>';
															echo '<path d="M73.716 63.684l19.192-38.735a8 8 0 0 1 14.347.022l19.072 38.794 42.77 6.282a8 8 0 0 1 4.413 13.652l-31.002 30.126 7.242 42.618a8 8 0 0 1-11.62 8.416l-38.233-20.175-38.294 20.057a8 8 0 0 
																1-11.594-8.451l7.373-42.596-30.909-30.221a8 8 0 0 1 4.455-13.639z">';
															echo '</path>';
														echo '</g>';
													echo '</svg>';
													echo '<svg viewBox="0 0 200 200" class="ikon-rating" id="ico_rep_verygood">';
														echo '<g data-name="Layer 54">';
															echo '<path fill="none" d="M0 0h200v200H0z"></path>';
															echo '<path d="M73.716 63.684l19.192-38.735a8 8 0 0 1 14.347.022l19.072 38.794 42.77 6.282a8 8 0 0 1 4.413 13.652l-31.002 30.126 7.242 42.618a8 8 0 0 1-11.62 8.416l-38.233-20.175-38.294 20.057a8 8 0 0 
																1-11.594-8.451l7.373-42.596-30.909-30.221a8 8 0 0 1 4.455-13.639z">';
															echo '</path>';
														echo '</g>';
													echo '</svg>';
													echo '<svg viewBox="0 0 200 200" class="ikon-rating" id="ico_rep_verygood">';
														echo '<g data-name="Layer 54">';
															echo '<path fill="none" d="M0 0h200v200H0z"></path>';
															echo '<path d="M73.716 63.684l19.192-38.735a8 8 0 0 1 14.347.022l19.072 38.794 42.77 6.282a8 8 0 0 1 4.413 13.652l-31.002 30.126 7.242 42.618a8 8 0 0 1-11.62 8.416l-38.233-20.175-38.294 20.057a8 8 0 0 
																1-11.594-8.451l7.373-42.596-30.909-30.221a8 8 0 0 1 4.455-13.639z">';
															echo '</path>';
														echo '</g>';
													echo '</svg>';
													echo '<svg viewBox="0 0 200 200" class="ikon-rating" id="ico_rep_verygood">';
														echo '<g data-name="Layer 54">';
															echo '<path fill="none" d="M0 0h200v200H0z"></path>';
															echo '<path d="M73.716 63.684l19.192-38.735a8 8 0 0 1 14.347.022l19.072 38.794 42.77 6.282a8 8 0 0 1 4.413 13.652l-31.002 30.126 7.242 42.618a8 8 0 0 1-11.62 8.416l-38.233-20.175-38.294 20.057a8 8 0 0 
																1-11.594-8.451l7.373-42.596-30.909-30.221a8 8 0 0 1 4.455-13.639z">';
															echo '</path>';
														echo '</g>';
													echo '</svg>';
													echo '<svg viewBox="0 0 200 200" class="ikon-rating" id="ico_rep_verygood">';
														echo '<g data-name="Layer 54">';
															echo '<path fill="none" d="M0 0h200v200H0z"></path>';
															echo '<path d="M73.716 63.684l19.192-38.735a8 8 0 0 1 14.347.022l19.072 38.794 42.77 6.282a8 8 0 0 1 4.413 13.652l-31.002 30.126 7.242 42.618a8 8 0 0 1-11.62 8.416l-38.233-20.175-38.294 20.057a8 8 0 0 
																1-11.594-8.451l7.373-42.596-30.909-30.221a8 8 0 0 1 4.455-13.639z">';
															echo '</path>';
														echo '</g>';
													echo '</svg>';
													echo '<svg viewBox="0 0 200 200" class="ikon-rating" id="ico_rep_verygood">';
														echo '<g data-name="Layer 54">';
															echo '<path fill="none" d="M0 0h200v200H0z"></path>';
															echo '<path d="M73.716 63.684l19.192-38.735a8 8 0 0 1 14.347.022l19.072 38.794 42.77 6.282a8 8 0 0 1 4.413 13.652l-31.002 30.126 7.242 42.618a8 8 0 0 1-11.62 8.416l-38.233-20.175-38.294 20.057a8 8 0 0 
																1-11.594-8.451l7.373-42.596-30.909-30.221a8 8 0 0 1 4.455-13.639z">';
															echo '</path>';
														echo '</g>';
													echo '</svg>';
													echo '<svg viewBox="0 0 200 200" class="ikon-rating" id="ico_rep_verygood">';
														echo '<g data-name="Layer 54">';
															echo '<path fill="none" d="M0 0h200v200H0z"></path>';
															echo '<path d="M73.716 63.684l19.192-38.735a8 8 0 0 1 14.347.022l19.072 38.794 42.77 6.282a8 8 0 0 1 4.413 13.652l-31.002 30.126 7.242 42.618a8 8 0 0 1-11.62 8.416l-38.233-20.175-38.294 20.057a8 8 0 0 
																1-11.594-8.451l7.373-42.596-30.909-30.221a8 8 0 0 1 4.455-13.639z">';
															echo '</path>';
														echo '</g>';
													echo '</svg>';
													echo '<svg viewBox="0 0 200 200" class="ikon-rating" id="ico_rep_verygood">';
														echo '<g data-name="Layer 54">';
															echo '<path fill="none" d="M0 0h200v200H0z"></path>';
															echo '<path d="M73.716 63.684l19.192-38.735a8 8 0 0 1 14.347.022l19.072 38.794 42.77 6.282a8 8 0 0 1 4.413 13.652l-31.002 30.126 7.242 42.618a8 8 0 0 1-11.62 8.416l-38.233-20.175-38.294 20.057a8 8 0 0 
																1-11.594-8.451l7.373-42.596-30.909-30.221a8 8 0 0 1 4.455-13.639z">';
															echo '</path>';
														echo '</g>';
													echo '</svg>';
													echo '<span class="star-icons-fill" style="width: '.$rating_brend.'%">';
														echo '<svg viewBox="0 0 200 200" class="ikon-rating" id="ico_rep_verygood">';
															echo '<g data-name="Layer 54">';
																echo '<path fill="none" d="M0 0h200v200H0z"></path>';
																echo '<path d="M73.716 63.684l19.192-38.735a8 8 0 0 1 14.347.022l19.072 38.794 42.77 6.282a8 8 0 0 1 4.413 13.652l-31.002 30.126 7.242 42.618a8 8 0 0 1-11.62 8.416l-38.233-20.175-38.294 20.057a8 8 0 0 
																	1-11.594-8.451l7.373-42.596-30.909-30.221a8 8 0 0 1 4.455-13.639z">';
																echo '</path>';
															echo '</g>';
														echo '</svg>';
														echo '<svg viewBox="0 0 200 200" class="ikon-rating" id="ico_rep_verygood">';
															echo '<g data-name="Layer 54">';
																echo '<path fill="none" d="M0 0h200v200H0z"></path>';
																echo '<path d="M73.716 63.684l19.192-38.735a8 8 0 0 1 14.347.022l19.072 38.794 42.77 6.282a8 8 0 0 1 4.413 13.652l-31.002 30.126 7.242 42.618a8 8 0 0 1-11.62 8.416l-38.233-20.175-38.294 20.057a8 8 0 0 
																	1-11.594-8.451l7.373-42.596-30.909-30.221a8 8 0 0 1 4.455-13.639z">';
																echo '</path>';
															echo '</g>';
														echo '</svg>';
														echo '<svg viewBox="0 0 200 200" class="ikon-rating" id="ico_rep_verygood">';
															echo '<g data-name="Layer 54">';
																echo '<path fill="none" d="M0 0h200v200H0z"></path>';
																echo '<path d="M73.716 63.684l19.192-38.735a8 8 0 0 1 14.347.022l19.072 38.794 42.77 6.282a8 8 0 0 1 4.413 13.652l-31.002 30.126 7.242 42.618a8 8 0 0 1-11.62 8.416l-38.233-20.175-38.294 20.057a8 8 0 0 
																	1-11.594-8.451l7.373-42.596-30.909-30.221a8 8 0 0 1 4.455-13.639z">';
																echo '</path>';
															echo '</g>';
														echo '</svg>';
														echo '<svg viewBox="0 0 200 200" class="ikon-rating" id="ico_rep_verygood">';
															echo '<g data-name="Layer 54">';
																echo '<path fill="none" d="M0 0h200v200H0z"></path>';
																echo '<path d="M73.716 63.684l19.192-38.735a8 8 0 0 1 14.347.022l19.072 38.794 42.77 6.282a8 8 0 0 1 4.413 13.652l-31.002 30.126 7.242 42.618a8 8 0 0 1-11.62 8.416l-38.233-20.175-38.294 20.057a8 8 0 0 
																	1-11.594-8.451l7.373-42.596-30.909-30.221a8 8 0 0 1 4.455-13.639z">';
																echo '</path>';
															echo '</g>';
														echo '</svg>';
														echo '<svg viewBox="0 0 200 200" class="ikon-rating" id="ico_rep_verygood">';
															echo '<g data-name="Layer 54">';
																echo '<path fill="none" d="M0 0h200v200H0z"></path>';
																echo '<path d="M73.716 63.684l19.192-38.735a8 8 0 0 1 14.347.022l19.072 38.794 42.77 6.282a8 8 0 0 1 4.413 13.652l-31.002 30.126 7.242 42.618a8 8 0 0 1-11.62 8.416l-38.233-20.175-38.294 20.057a8 8 0 0 
																	1-11.594-8.451l7.373-42.596-30.909-30.221a8 8 0 0 1 4.455-13.639z">';
																echo '</path>';
															echo '</g>';
														echo '</svg>';
														echo '<svg viewBox="0 0 200 200" class="ikon-rating" id="ico_rep_verygood">';
															echo '<g data-name="Layer 54">';
																echo '<path fill="none" d="M0 0h200v200H0z"></path>';
																echo '<path d="M73.716 63.684l19.192-38.735a8 8 0 0 1 14.347.022l19.072 38.794 42.77 6.282a8 8 0 0 1 4.413 13.652l-31.002 30.126 7.242 42.618a8 8 0 0 1-11.62 8.416l-38.233-20.175-38.294 20.057a8 8 0 0 
																	1-11.594-8.451l7.373-42.596-30.909-30.221a8 8 0 0 1 4.455-13.639z">';
																echo '</path>';
															echo '</g>';
														echo '</svg>';
														echo '<svg viewBox="0 0 200 200" class="ikon-rating" id="ico_rep_verygood">';
															echo '<g data-name="Layer 54">';
																echo '<path fill="none" d="M0 0h200v200H0z"></path>';
																echo '<path d="M73.716 63.684l19.192-38.735a8 8 0 0 1 14.347.022l19.072 38.794 42.77 6.282a8 8 0 0 1 4.413 13.652l-31.002 30.126 7.242 42.618a8 8 0 0 1-11.62 8.416l-38.233-20.175-38.294 20.057a8 8 0 0 
																	1-11.594-8.451l7.373-42.596-30.909-30.221a8 8 0 0 1 4.455-13.639z">';
																echo '</path>';
															echo '</g>';
														echo '</svg>';
														echo '<svg viewBox="0 0 200 200" class="ikon-rating" id="ico_rep_verygood">';
															echo '<g data-name="Layer 54">';
																echo '<path fill="none" d="M0 0h200v200H0z"></path>';
																echo '<path d="M73.716 63.684l19.192-38.735a8 8 0 0 1 14.347.022l19.072 38.794 42.77 6.282a8 8 0 0 1 4.413 13.652l-31.002 30.126 7.242 42.618a8 8 0 0 1-11.62 8.416l-38.233-20.175-38.294 20.057a8 8 0 0 
																	1-11.594-8.451l7.373-42.596-30.909-30.221a8 8 0 0 1 4.455-13.639z">';
																echo '</path>';
															echo '</g>';
														echo '</svg>';
														echo '<svg viewBox="0 0 200 200" class="ikon-rating" id="ico_rep_verygood">';
															echo '<g data-name="Layer 54">';
																echo '<path fill="none" d="M0 0h200v200H0z"></path>';
																echo '<path d="M73.716 63.684l19.192-38.735a8 8 0 0 1 14.347.022l19.072 38.794 42.77 6.282a8 8 0 0 1 4.413 13.652l-31.002 30.126 7.242 42.618a8 8 0 0 1-11.62 8.416l-38.233-20.175-38.294 20.057a8 8 0 0 
																	1-11.594-8.451l7.373-42.596-30.909-30.221a8 8 0 0 1 4.455-13.639z">';
																echo '</path>';
															echo '</g>';
														echo '</svg>';
														echo '<svg viewBox="0 0 200 200" class="ikon-rating" id="ico_rep_verygood">';
															echo '<g data-name="Layer 54">';
																echo '<path fill="none" d="M0 0h200v200H0z"></path>';
																echo '<path d="M73.716 63.684l19.192-38.735a8 8 0 0 1 14.347.022l19.072 38.794 42.77 6.282a8 8 0 0 1 4.413 13.652l-31.002 30.126 7.242 42.618a8 8 0 0 1-11.62 8.416l-38.233-20.175-38.294 20.057a8 8 0 0 
																	1-11.594-8.451l7.373-42.596-30.909-30.221a8 8 0 0 1 4.455-13.639z">';
																echo '</path>';
															echo '</g>';
														echo '</svg>';
													echo '</span>';
												echo '</span>';
											echo '</div>';
											echo '<div class="page-list-item-cont">';
												echo '<div class="item-cont-text-page">';
													echo '<span class="item-cont-text-span">'.$text_brend.'</span>';
												echo '</div>';
												echo '<div class="tc-apply page">';
													echo '<div class="tc-apply-title">';
														echo '<div class="tc-apply-ikon" style="background-image: url('.get_template_directory_uri().'/img/question.svg);"></div>';
														echo '<span class="tc-apply-text">T&C’s Apply</span>';
													echo '</div>';
													echo '<div id="tcapply" class="page-tcapply">'.$hover_text.'</div>';
												echo '</div>';
											echo '</div>';
											if ($advant_brend_one && $advant_brend_two && $advant_brend_three) {
												echo '<div class="page-list-item-advantage">';
													echo '<ul>';
														echo '<li>';
															echo '<div class="ikon-advantage" style="background-image: url('.get_template_directory_uri().'/img/advantage.svg);"></div>';
															echo '<span>'.$advant_brend_one.'</span>';
														echo '</li>';
														echo '<li>';
															echo '<div class="ikon-advantage" style="background-image: url('.get_template_directory_uri().'/img/advantage.svg);"></div>';
															echo '<span>'.$advant_brend_two.'</span>';
														echo '</li>';
														echo '<li>';
															echo '<div class="ikon-advantage" style="background-image: url('.get_template_directory_uri().'/img/advantage.svg);"></div>';
															echo '<span>'.$advant_brend_three.'</span>';
														echo '</li>';
													echo '</ul>';
												echo '</div>';
											} 
											echo '<div class="page-button-block-list">';
												echo '<div class="block-flex-btn">';
													echo '<a href="'.$link_brend.'" class="flex-btn">';
														echo '<div class="btn-ikon" style="background-image: url('.get_template_directory_uri().'/img/play.svg);"></div>';
														echo '<span>Jetzt spielen</span>';
													echo '</a>';
												echo '</div>';
												echo '<div class="block-overview">';
													echo '<a href="'.get_permalink($item->ID).'" class="link-overview page">';
														echo '<img class="overview-ikon img-svg" src="'.get_template_directory_uri().'/img/bewertung.svg" alt="Bewertung">';
														echo '<span>Bewertung</span>';
													echo '</a>';
												echo '</div>';
											echo '</div>';
										echo '</div>';
									echo '</div>';
									echo '<div class="card-page-list-others">';
										echo '<div class="card-page-others">';
											echo '<div class="card-page-games">';
												echo '<div class="card-page-games-title">';
													echo '<span>Verfügbare Spiele</span>';
												echo '</div>';
												echo '<div class="card-page-games-content">';
													echo '<div class="page-games-item">';
														echo '<svg class="page-games-ikon '.$delete_blackjack.'" id="Layer_5" enable-background="new 0 0 64 64" height="64" viewBox="0 0 64 64" width="64" xmlns="http://www.w3.org/2000/svg">';
															echo '<g>';
																echo '<path d="m33.615 39.799-3.615 3.615-7.465-7.465c-.99-.99-1.535-2.306-1.535-3.706 0-2.891 2.352-5.243 5.242-5.243 1.401 0 2.718.545 3.708 1.536l.05.05.051-.051c.808-.808 1.837-1.3 2.949-1.461v-10.074h-15c-.552 0-1 .449-1 1v19.363c5.724 1.36 10 6.503 10 12.637 0 1.034-.134 2.035-.363 3h15.363c.552 0 1-.448 1-1v-11h-7c-.977 0-1.837-.476-2.385-1.201zm-14.615-20.799h6v2h-4v4h-2zm20 26h2v6h-6v-2h4z"/>';
																echo '<path d="m16 49h-1v-1c0-.552-.448-1-1-1s-1 .448-1 1v1h-1c-.552 0-1 .448-1 1s.448 1 1 1 1-.448 1-1h2c0 .552.448 1 1 1s1-.448 1-1-.448-1-1-1z"/>';
																echo '<path d="m6.966 58.448c1.667 1.391 3.751 2.294 6.034 2.501v-2.008c-1.733-.193-3.316-.88-4.611-1.916z"/>';
																echo '<path d="m54.75 15-1.75-2.333-1.75 2.333 1.75 2.333z"/>';
																echo '<path d="m15 58.941v2.008c2.283-.207 4.366-1.111 6.034-2.501l-1.423-1.423c-1.295 1.036-2.878 1.723-4.611 1.916z"/>';
																echo '<path d="m21 50c0-3.859-3.141-7-7-7s-7 3.141-7 7 3.141 7 7 7 7-3.141 7-7zm-5 3c-.352 0-.686-.067-1-.179v2.179h-2v-2.179c-.314.112-.648.179-1 .179-1.654 0-3-1.346-3-3 0-1.352.898-2.497 2.129-2.871.374-1.231 1.519-2.129 2.871-2.129s2.497.898 2.871 2.129c1.231.374 2.129 1.519 2.129 2.871 0 1.654-1.346 3-3 3z"/>';
																echo '<path d="m21.025 44.389c1.036 1.295 1.723 2.878 1.916 4.611h2.008c-.207-2.283-1.111-4.366-2.501-6.034z"/>';
																echo '<path d="m21.025 55.611 1.423 1.423c1.391-1.667 2.294-3.751 2.501-6.034h-2.008c-.193 1.733-.88 3.316-1.916 4.611z"/>';
																echo '<path d="m28.535 29.95c-.612-.613-1.426-.95-2.293-.95-1.788 0-3.242 1.455-3.242 3.243 0 .854.346 1.689.949 2.292l6.051 6.051 3-3v-8.487c-.576.137-1.106.422-1.534.85l-1.466 1.465z"/>';
																echo '<path d="m21.034 41.552c-1.667-1.391-3.751-2.294-6.034-2.501v2.008c1.733.193 3.316.88 4.611 1.916z"/>';
																echo '<path d="m54.75 27-1.75-2.333-1.75 2.333 1.75 2.333z"/>';
																echo '<path d="m13 41.059v-2.008c-2.283.207-4.366 1.111-6.034 2.501l1.423 1.423c1.295-1.036 2.878-1.723 4.611-1.916z"/>';
																echo '<path d="m60 3h-24c-.552 0-1 .449-1 1v34c0 .552.448 1 1 1h24c.552 0 1-.448 1-1v-34c0-.551-.448-1-1-1zm-7 6.333 4.25 5.667-4.25 5.667-4.25-5.667zm-16-4.333h6v2h-4v4h-2zm6 27.667-4.25-5.667 4.25-5.667 4.25 5.667zm0-12-4.25-5.667 4.25-5.667 4.25 5.667zm10 .666 4.25 5.667-4.25 5.667-4.25-5.667zm6 15.667h-6v-2h4v-4h2z"/>';
																echo '<path d="m6.975 44.389-1.423-1.423c-1.391 1.668-2.294 3.751-2.501 6.034h2.008c.193-1.733.88-3.316 1.916-4.611z"/>';
																echo '<path d="m41.25 27 1.75 2.333 1.75-2.333-1.75-2.333z"/>';
																echo '<path d="m5.059 51h-2.008c.207 2.283 1.111 4.366 2.501 6.034l1.423-1.423c-1.036-1.295-1.723-2.878-1.916-4.611z"/>';
																echo '<path d="m41.25 15 1.75 2.333 1.75-2.333-1.75-2.333z"/>';
															echo '</g>';
														echo '</svg>';
														echo '<span class="page-games-text '.$delete_blackjack.'">Blackjack</span>';
													echo '</div>';
													echo '<div class="page-games-item">';
														echo '<svg class="page-games-ikon '.$delete_slots.'" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="64" height="64" viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve">';
															echo '<g>';
																echo '<g>';
																	echo '<path d="M130,452c-5.52,0-10,4.48-10,10c0,5.52,4.48,10,10,10s10-4.48,10-10C140,456.48,135.52,452,130,452z"/>';
																echo '</g>';
															echo '</g>';
															echo '<g>';
																echo '<g>';
																	echo '<path d="M98.112,270.153C96.233,267.545,93.214,266,90,266H50c-5.523,0-10,4.478-10,10c0,5.522,4.477,10,10,10h26.126
																		l-15.613,46.838c-1.747,5.239,1.085,10.902,6.325,12.649c1.049,0.35,2.115,0.516,3.164,0.516c4.187,0,8.089-2.65,9.485-6.841
																		l20-60C100.504,276.113,99.992,272.761,98.112,270.153z"/>';
																echo '</g>';
															echo '</g>';
															echo '<g>';
																echo '<g>';
																	echo '<path d="M218.112,270.153C216.233,267.545,213.214,266,210,266h-40c-5.523,0-10,4.478-10,10c0,5.522,4.477,10,10,10h26.126
																		l-15.613,46.838c-1.747,5.239,1.085,10.902,6.325,12.649c1.049,0.35,2.115,0.516,3.164,0.516c4.187,0,8.089-2.65,9.485-6.841
																		l20-60C220.504,276.113,219.992,272.761,218.112,270.153z"/>';
																echo '</g>';
															echo '</g>';
															echo '<g>';
																echo '<g>';
																	echo '<path d="M338.112,270.153C336.233,267.545,333.214,266,330,266h-40c-5.523,0-10,4.478-10,10c0,5.522,4.477,10,10,10h26.126
																		l-15.613,46.838c-1.747,5.239,1.085,10.902,6.325,12.649c1.049,0.35,2.115,0.516,3.164,0.516c4.187,0,8.089-2.65,9.485-6.841
																		l20-60C340.503,276.113,339.992,272.761,338.112,270.153z"/>';
																echo '</g>';
															echo '</g>';
															echo '<g>';
																echo '<g>';
																	echo '<path d="M250,452h-80c-5.523,0-10,4.478-10,10c0,5.522,4.477,10,10,10h80c5.523,0,10-4.478,10-10C260,456.478,255.523,452,250,452
															z"/>';
																echo '</g>';
															echo '</g>';
															echo '<g>';
																echo '<g>';
																	echo '<path d="M462,166c-27.57,0-50,22.43-50,50c0,15.78,7.543,30.604,20,39.98V369h-52V233v-43C380,85.233,294.766,0,190,0
																		S0,85.233,0,190v43v249c0,16.542,13.458,30,30,30h60h200h60c16.542,0,30-13.458,30-30v-30h62c27.57,0,50-22.43,50-50V255.98
																		c12.457-9.376,20-24.2,20-39.98C512,188.43,489.57,166,462,166z M20,243h100v126H20V243z M280,492H100v-60h180V492z M360,442v40
																		c0,5.514-4.486,10-10,10h-50v-70c0-5.522-4.477-10-10-10H90c-5.523,0-10,4.478-10,10v70H30c-5.514,0-10-4.486-10-10v-93h110h120
																		h110V442z M140,369V243h100v126H140z M360,369H260V243h100V369z M360,223H20v-33C20,96.262,96.262,20,190,20s170,76.262,170,170
																		V223z M432,432h-52v-43h52V432z M472,402c0,13.036-8.361,24.152-20,28.28V379v-73h20V402z M472,286h-20v-21.017
																		c3.28,0.669,6.63,1.017,10,1.017s6.72-0.348,10-1.017V286z M477.001,241.989C472.455,244.613,467.268,246,462,246
																		s-10.455-1.387-15.001-4.011C437.747,236.632,432,226.674,432,216c0-16.542,13.458-30,30-30s30,13.458,30,30
																		C492,226.678,486.249,236.639,477.001,241.989z"/>';
																echo '</g>';
															echo '</g>';
															echo '<g>';
																echo '<g>';
																	echo '<path d="M190,40c-82.71,0-150,67.29-150,150c0,5.522,4.477,10,10,10h100h180c5.523,0,10-4.478,10-10C340,107.29,272.71,40,190,40z
																		M150,180H60.381C65.504,112.972,121.682,60,190,60s124.496,52.973,129.619,120H150z"/>';
																echo '</g>';
															echo '</g>';
															echo '<g>';
																echo '<g>';
																	echo ' <path d="M190,80c-5.523,0-10,4.478-10,10v40c0,5.522,4.477,10,10,10s10-4.478,10-10V90C200,84.478,195.523,80,190,80z"/>';
																echo '</g>';
															echo '</g>';
															echo '<g>';
																echo '<g>';
																	echo '<path d="M154.641,140.499l-28.28-28.28c-3.905-3.903-10.237-3.904-14.142,0c-3.905,3.905-3.906,10.237,0,14.143l28.28,28.28
																		c1.953,1.952,4.512,2.929,7.071,2.929s5.119-0.977,7.071-2.929C158.546,150.737,158.547,144.405,154.641,140.499z"/>';
																echo '</g>';
															echo '</g>';
															echo '<g>';
																echo '<g>';
																	echo '<path d="M267.781,112.218c-3.905-3.904-10.237-3.904-14.142,0l-28.28,28.28c-3.905,3.905-3.905,10.237,0,14.143
																		c1.953,1.953,4.512,2.929,7.071,2.929c2.559,0,5.119-0.977,7.071-2.929l28.28-28.28
																		C271.686,122.456,271.686,116.124,267.781,112.218z"/>';
																echo '</g>';
															echo '</g>';
														echo ' </svg>';
														echo '<span class="page-games-text '.$delete_slots.'">Slots</span>';
													echo '</div>';
													echo '<div class="page-games-item">';
														echo '<svg class="page-games-ikon '.$delete_bingo.'" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="64" height="64" viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve">';
															echo '<g>';
																echo '<g>';
																	echo '<path d="M276,106c-5.52,0-10,4.48-10,10s4.48,10,10,10c5.52,0,10-4.48,10-10S281.52,106,276,106z"/>';
																echo '</g>';
															echo '</g>';
															echo '<g>';
																echo '<g>';
																	echo '<path d="M270,180c-5.52,0-10,4.48-10,10s4.48,10,10,10c5.52,0,10-4.48,10-10S275.52,180,270,180z"/>';
																echo '</g>';
															echo '</g>';
															echo '<g>';
																echo '<g>';
																	echo '<path d="M110,100c-5.52,0-10,4.48-10,10s4.48,10,10,10s10-4.48,10-10S115.52,100,110,100z"/>';
																echo '</g>';
															echo '</g>';
															echo '<g>';
																echo '<g>';
																	echo '<path d="M110,180c-5.52,0-10,4.48-10,10s4.48,10,10,10s10-4.48,10-10S115.52,180,110,180z"/>';
																echo '</g>';
															echo '</g>';
															echo '<g>';
																echo '<g>';
																	echo '<path d="M110,340c-5.52,0-10,4.48-10,10c0,5.52,4.48,10,10,10s10-4.48,10-10C120,344.48,115.52,340,110,340z"/>';
																echo '</g>';
															echo '</g>';
															echo '<g>';
																echo '<g>';
																	echo '<path d="M442,412h-10v-30c0-5.523-4.477-10-10-10s-10,4.477-10,10v30h-33.82l32.764-65.528c2.47-4.94,0.468-10.947-4.472-13.417
																		c-4.939-2.47-10.947-0.468-13.417,4.472l-40,80c-1.55,3.1-1.384,6.781,0.438,9.729c1.822,2.948,5.041,4.743,8.506,4.743h50v30
																		c0,5.523,4.477,10,10,10s10-4.477,10-10v-30h10c5.523,0,10-4.477,10-10S447.523,412,442,412z"/>';
															echo '</g>';
															echo '</g>';
															echo '<g>';
															echo '<g>';
																echo '<path d="M402,292c-7.432,0-14.79,0.742-22,2.205V30c0-16.542-13.458-30-30-30H30C13.458,0,0,13.458,0,30v400
																	c0,16.542,13.458,30,30,30h278.531c20.015,32.22,55.276,52,93.469,52c60.654,0,110-49.346,110-110S462.654,292,402,292z
																		M300,360.799c-2.516,6.22-4.452,12.645-5.781,19.201H240v-60h60V360.799z M300,300h-60v-60h60V300z M300,220h-60v-60h60V220z
																		M70,140c-5.523,0-10,4.477-10,10v240c0,5.523,4.477,10,10,10h222.03c-0.012,0.667-0.03,1.334-0.03,2
																	c0,12.992,2.322,25.887,6.768,38H30c-5.514,0-10-4.486-10-10V30c0-5.514,4.486-10,10-10h320c5.514,0,10,4.486,10,10v270.309
																	c-15.39,6.359-29.037,16.103-40,28.362V150c0-5.523-4.477-10-10-10H70z M160,300v-60h60v60H160z M220,320v60h-60v-60H220z
																		M160,220v-60h60v60H160z M80,300v-60h60v60H80z M140,320v60H80v-60H140z M80,220v-60h60v60H80z M402,492
																	c-54.147,0-95.773-47.325-89.359-100.75c3.959-33.591,27.092-63.283,60.614-74.571c9.241-3.105,18.913-4.68,28.745-4.68
																	c49.626,0,90,40.374,90,90C492,451.625,451.626,492,402,492z"/>';
																echo '</g>';
															echo '</g>';
															echo '<g>';
																echo '<g>';
																	echo '<path d="M310,60H70c-5.523,0-10,4.477-10,10s4.477,10,10,10h240c5.523,0,10-4.477,10-10S315.523,60,310,60z"/>';
																echo '</g>';
															echo '</g>';
															echo '<g>';
																echo '<g>';
																	echo '<path d="M230,100h-80c-5.523,0-10,4.477-10,10s4.477,10,10,10h80c5.523,0,10-4.477,10-10S235.523,100,230,100z"/>';
																echo '</g>';
															echo '</g>';
														echo '</svg>';
														echo '<span class="page-games-text '.$delete_bingo.'">Bingo</span>';
													echo '</div>';
													echo '<div class="page-games-item">';
														echo '<svg class="page-games-ikon '.$delete_roulette.'" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="64" height="64" viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve">';
															echo '<g>';
																echo '<g>';
																	echo '<path d="M326,126c-5.52,0-10,4.48-10,10s4.48,10,10,10c5.52,0,10-4.48,10-10S331.52,126,326,126z"/>';
																echo '</g>';
															echo '</g>';
															echo '<g>';
																echo '<g>';
																	echo '<path d="M256,0C117.577,0,0,118.261,0,256c0,138.227,118.062,256,256,256c91.709,0,174.606-52.029,219.907-127.578
																		C499.519,345.035,512,300.628,512,256C512,117.629,393.793,0,256,0z M491.765,246.001h-43.063V246
																		c-1.475-27.167-8.937-53.906-21.863-78.35l36.792-21.245C480.575,177.491,490.199,211.563,491.765,246.001z M453.319,129.263
																		l-36.722,21.205c-14.524-21.754-33.312-40.542-55.065-55.065l21.205-36.722C410.757,77.114,434.887,101.243,453.319,129.263z
																			M266,20.235c34.439,1.566,68.512,11.19,99.596,28.134l-21.245,36.793C319.907,72.235,293.168,64.774,266,63.298V20.235z
																			M246,20.235v43.063c-27.168,1.475-53.907,8.937-78.351,21.863L146.404,48.37C177.489,31.425,211.561,21.801,246,20.235z
																			M129.263,58.681l21.205,36.722c-21.753,14.523-40.541,33.312-55.065,55.065l-36.722-21.205
																		C77.114,101.243,101.244,77.113,129.263,58.681z M48.369,146.404l36.792,21.245C72.235,192.094,64.774,218.833,63.298,246H20.235
																		C21.801,211.562,31.426,177.49,48.369,146.404z M20.235,266h43.063c1.475,27.167,8.937,53.906,21.863,78.35l-36.792,21.246
																		C31.425,334.51,21.801,300.438,20.235,266z M58.681,382.737l36.723-21.205c14.524,21.754,33.312,40.542,55.065,55.065
																		l-21.205,36.722C101.243,434.886,77.113,410.757,58.681,382.737z M246,491.764c-34.439-1.566-68.512-11.19-99.596-28.134
																		l21.245-36.792c24.444,12.926,51.183,20.388,78.351,21.863V491.764z M83,256c0-94.073,79.671-173,173-173
																		c94.153,0,173,79.854,173,173c0,93.768-79.309,173-173,173C162.221,429,83,349.678,83,256z M266,491.766v-43.064
																		c27.167-1.475,53.907-8.937,78.351-21.863l21.245,36.792C334.511,480.576,300.439,490.2,266,491.766z M382.737,453.319
																		l-21.205-36.722c21.752-14.522,40.539-33.309,55.063-55.061l36.725,21.202C434.886,410.758,410.756,434.887,382.737,453.319z
																			M463.63,365.597l-36.794-21.242c12.928-24.446,20.39-51.287,21.866-78.455h43.063
																		C490.199,300.339,480.574,334.511,463.63,365.597z"/>';
																echo '</g>';
															echo '</g>';
															echo '<g>';
																echo '<g>';
																	echo '<path d="M399.762,213.137c-6.635-22.203-18.589-42.913-34.57-59.892c-3.785-4.021-10.114-4.214-14.136-0.427
																		c-4.021,3.785-4.213,10.113-0.428,14.136c13.859,14.723,24.223,32.673,29.971,51.909c1.579,5.281,7.14,8.302,12.445,6.718
																		C398.336,224,401.343,218.429,399.762,213.137z"/>';
																echo '</g>';
															echo '</g>';
															echo '<g>';
																echo '<g>';
																	echo '<path d="M356,226c-13.036,0-24.152,8.361-28.28,20h-22.726c-3.987-19.563-19.431-35.007-38.994-38.994V184.28
																		c11.639-4.128,20-15.243,20-28.28c0-16.542-13.458-30-30-30s-30,13.458-30,30c0,13.036,8.361,24.152,20,28.28v22.726
																		c-19.563,3.987-35.007,19.431-38.994,38.994H184.28c-4.128-11.639-15.243-20-28.28-20c-16.542,0-30,13.458-30,30s13.458,30,30,30
																		c13.036,0,24.152-8.361,28.28-20h22.726c3.987,19.563,19.431,35.007,38.994,38.994v22.726c-11.639,4.128-20,15.243-20,28.28
																		c0,16.542,13.458,30,30,30s30-13.458,30-30c0-13.036-8.361-24.152-20-28.28v-22.726c19.563-3.987,35.007-19.431,38.994-38.994
																		h22.726c4.128,11.639,15.243,20,28.28,20c16.542,0,30-13.458,30-30S372.542,226,356,226z M156,266c-5.514,0-10-4.486-10-10
																		c0-5.514,4.486-10,10-10c5.514,0,10,4.486,10,10C166,261.514,161.514,266,156,266z M256,146c5.514,0,10,4.486,10,10
																		c0,5.514-4.486,10-10,10c-5.514,0-10-4.486-10-10C246,150.486,250.486,146,256,146z M256,366c-5.514,0-10-4.486-10-10
																		c0-5.514,4.486-10,10-10c5.514,0,10,4.486,10,10C266,361.514,261.514,366,256,366z M256,286c-16.542,0-30-13.458-30-30
																		s13.458-30,30-30s30,13.458,30,30S272.542,286,256,286z M356,266c-5.514,0-10-4.486-10-10c0-5.514,4.486-10,10-10
																		c5.514,0,10,4.486,10,10C366,261.514,361.514,266,356,266z"/>';
																echo '</g>';
															echo '</g>';
														echo '</svg>';
														echo '<span class="page-games-text '.$delete_roulette.'">Roulette</span>';
													echo '</div>';
													echo '<div class="page-games-item">';
														echo '<svg class="page-games-ikon '.$delete_baccarat.'" height="64" viewBox="0 0 64 64" width="64" xmlns="http://www.w3.org/2000/svg">';
															echo '<path d="m61 11h-13a1 1 0 0 0 -.707.293l-1.707 1.707h-27.172l-1.707-1.707a1 1 0 0 0 -.707-.293h-13a1 1 0 0 0 -1 1v40a1 1 0 0 0 1 1h58a1 1 0 0 0 1-1v-40a1 1 0 0 0 -1-1zm-22 4h4.586l-2 2h-2.586zm-4 0h2v2h-2zm-4 0h2v2h-2zm-4 0h2v2h-2zm-2 0v2h-2.586l-2-2zm35 36h-56v-38h11.586l5.707 5.707a1 1 0 0 0 .707.293h20a1 1 0 0 0 .707-.293l5.707-5.707h11.586z"/>';
															echo '<path d="m43 21h-22v8h22zm-2 6h-18v-4h18z"/>';
															echo '<path d="m31 31h2v5h-2z"/>';
															echo '<path d="m55.906 23h-9a1 1 0 0 0 -1 1 13.933 13.933 0 0 1 -2.316 7.7 14.1 14.1 0 0 1 -6.928 5.448 13.53 13.53 0 0 1 -9.413 0 14.105 14.105 0 0 1 -6.93-5.449 13.93 13.93 0 0 1 -2.319-7.699 1 1 0 0 0 -1-1h-9a1 1 0 0 0 -1 1 25 25 0 0 0 24.888 25h.13a25 25 0 0 0 22.244-13.812 24.733 24.733 0 0 0 2.644-11.188 1 1 0 0 0 -1-1zm-46.885 2h7.01a15.906 15.906 0 0 0 1.916 6.643l-6.87 1.908a22.7 22.7 0 0 1 -2.056-8.551zm2.987 10.369 7.058-1.96a16.153 16.153 0 0 0 6.08 5.031l-5.166 5.165a22.979 22.979 0 0 1 -7.972-8.236zm18.9 11.6a23.01 23.01 0 0 1 -9.1-2.359l5.372-5.372a15.984 15.984 0 0 0 3.733.718zm2.094 0v-7.01a15.984 15.984 0 0 0 3.733-.718l5.372 5.372a23.01 23.01 0 0 1 -9.107 2.359zm10.926-3.367-5.167-5.162a16.15 16.15 0 0 0 6.079-5.031l7.058 1.96a22.962 22.962 0 0 1 -7.972 8.236zm8.9-10.054-6.87-1.908a15.914 15.914 0 0 0 1.917-6.64h7.01a22.7 22.7 0 0 1 -2.056 8.551z"/>';
															echo '<path d="m12 28h2v2h-2z"/><path d="m17 37h2v2h-2z"/>';
															echo '<path d="m27 42h2v2h-2z"/><path d="m35 42h2v2h-2z"/>';
															echo '<path d="m44 37h2v2h-2z"/><path d="m50 28h2v2h-2z"/>';
															echo '<path d="m8 17h4v-2h-6v6h2z"/>';
															echo '<path d="m56 21h2v-6h-6v2h4z"/>';
														echo '</svg>';
														echo '<span class="page-games-text '.$delete_baccarat.'">Baccarat</span>';
													echo '</div>';
													echo '<div class="page-games-item">';
														echo '<svg class="page-games-ikon '.$delete_poker.'" id="Layer_3" enable-background="new 0 0 64 64" height="64" viewBox="0 0 64 64" width="64" xmlns="http://www.w3.org/2000/svg">';
															echo '<g>';
																echo '<path d="m18 5h8v2h-8z"/>';
																echo '<path d="m17 40c0 4.962 4.037 9 9 9s9-4.038 9-9-4.037-9-9-9-9 4.038-9 
																	9zm10-6.92v2.021c-.323-.066-.658-.101-1-.101s-.677.035-1 .101v-2.021c.328-.047.66-.08 1-.08s.672.033 1 
																	.08zm6 6.92c0 .34-.033.672-.08 1h-2.021c.066-.323.101-.658.101-1s-.035-.677-.101-1h2.021c.047.328.08.66.08 
																	1zm-14 0c0-.34.033-.672.08-1h2.021c-.066.323-.101.658-.101 1s.035.677.101 1h-2.021c-.047-.328-.08-.66-.08-1zm4 
																	0c0-1.654 1.346-3 3-3s3 1.346 3 3-1.346 3-3 3-3-1.346-3-3zm0-6.315v2.34c-.369.279-.695.606-.974.974h-2.34c.691-1.449 
																	1.864-2.622 3.314-3.314zm-3.315 9.315h2.34c.279.369.606.695.974.974v2.34c-1.449-.691-2.622-1.864-3.314-3.314zm5.315 
																	3.92v-2.021c.323.066.658.101 1 .101s.677-.035 1-.101v2.021c-.328.047-.66.08-1 .08s-.672-.033-1-.08zm4-.605v-2.34c.369-.279.695-.606.974-.974h2.34c-.691 
																	1.449-1.864 2.622-3.314 3.314zm3.315-9.315h-2.34c-.279-.369-.606-.695-.974-.974v-2.34c1.449.691 2.622 1.864 3.314 3.314z"/>';
																echo '<path d="m47 22c0-4.624-3.507-8.442-8-8.941v-6.059c0-3.309-2.691-6-6-6h-22c-3.309 0-6 2.691-6 6v50c0 3.309 2.691 6 6 
																	6h22c3.309 0 6-2.691 6-6v-26.059c4.493-.499 8-4.317 8-8.941zm-2 0c0 .34-.033.672-.08 
																	1h-2.021c.066-.323.101-.658.101-1s-.035-.677-.101-1h2.021c.047.328.08.66.08 1zm-13.315 
																	3h2.34c.279.369.606.695.974.974v2.34c-1.449-.691-2.622-1.864-3.314-3.314zm9.315-3c0 1.654-1.346 3-3 3s-3-1.346-3-3 1.346-3 
																	3-3 3 1.346 3 3zm-6.974-3h-2.34c.692-1.45 1.865-2.623 3.315-3.315v2.34c-.37.28-.696.606-.975.975zm-3.026 
																	3c0-.34.033-.672.08-1h2.021c-.066.323-.101.658-.101 1s.035.677.101 1h-2.021c-.047-.328-.08-.66-.08-1zm6 
																	4.899c.323.066.658.101 1 .101s.677-.035 1-.101v2.021c-.328.047-.66.08-1 .08s-.672-.033-1-.08zm1-9.899c-.342 0-.677.035-1 
																	.101v-2.021c.328-.047.66-.08 1-.08s.672.033 1 .08v2.021c-.323-.066-.658-.101-1-.101zm-9 5c0 4.624 3.507 8.442 8 
																	8.941v20.059h-30v-40h30v2.059c-4.493.499-8 4.317-8 8.941zm12 6.315v-2.34c.369-.279.695-.606.974-.974h2.34c-.691 
																	1.449-1.864 2.622-3.314 3.314zm3.315-9.315h-2.34c-.279-.369-.606-.695-.974-.974v-2.34c1.449.691 2.622 1.864 3.314 
																	3.314zm-33.315-16h22c2.206 0 4 1.794 4 4v2h-30v-2c0-2.206 1.794-4 4-4zm22 58h-22c-2.206 0-4-1.794-4-4v-4h30v4c0 2.206-1.794 
																	4-4 4z"/>';
																echo '<path d="m50 31c-4.963 0-9 4.038-9 9s4.037 9 9 9 9-4.038 9-9-4.037-9-9-9zm6.315 6h-2.34c-.279-.369-.606-.695-.974-.974v-2.34c1.449.691 
																	2.622 1.864 3.314 3.314zm.685 3c0 .34-.033.672-.08 1h-2.021c.066-.323.101-.658.101-1s-.035-.677-.101-1h2.021c.047.328.08.66.08 
																	1zm-14 0c0-.34.033-.672.08-1h2.021c-.066.323-.101.658-.101 1s.035.677.101 1h-2.021c-.047-.328-.08-.66-.08-1zm4 0c0-1.654 
																	1.346-3 3-3s3 1.346 3 3-1.346 3-3 3-3-1.346-3-3zm4-6.92v2.021c-.323-.066-.658-.101-1-.101s-.677.035-1 .101v-2.021c.328-.047.66-.08 
																	1-.08s.672.033 1 .08zm-4 .605v2.34c-.369.279-.695.606-.974.974h-2.34c.691-1.449 1.864-2.622 3.314-3.314zm-3.315 
																	9.315h2.34c.279.369.606.695.974.974v2.34c-1.449-.691-2.622-1.864-3.314-3.314zm5.315 3.92v-2.021c.323.066.658.101 1 
																	.101s.677-.035 1-.101v2.021c-.328.047-.66.08-1 .08s-.672-.033-1-.08zm4-.605v-2.34c.369-.279.695-.606.974-.974h2.34c-.691 
																	1.449-1.864 2.622-3.314 3.314z"/>';
																echo '<circle cx="22" cy="57" r="2"/>';
															echo '</g>';
														echo '</svg>';
														echo '<span class="page-games-text '.$delete_poker.'">Video-Poker</span>';
													echo '</div>';
													echo '<div class="page-games-item">';
														echo '<svg class="page-games-ikon '.$delete_rubbellose.'" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="64" height="64" viewBox="0 0 480 480" style="enable-background:new 0 0 480 480;" xml:space="preserve">';
															echo '<g>';
																echo '<g>';
																	echo '<path d="M472,72H8c-4.418,0-8,3.582-8,8v320c0,4.418,3.582,8,8,8h464c4.418,0,8-3.582,8-8V80C480,75.582,476.418,72,472,72z
																		M464,392H16V88h448V392z"/>';
																echo '</g>';
															echo '</g>';
															echo '<g>';
																echo '<g>';
																	echo '<path d="M440,104H168c-4.418,0-8,3.582-8,8v256c0,4.418,3.582,8,8,8h272c4.418,0,8-3.582,8-8V112C448,107.582,444.418,104,440,104
																		z M432,360H176V120h256V360z"/>';
																echo '</g>';
															echo '</g>';
															echo '<g>';
																echo '<g>';
																	echo '<path d="M272,232c-4.418,0-8-3.582-8-8s3.582-8,8-8s8,3.582,8,8h16c-0.042-10.134-6.445-19.15-16-22.528V192h-16v9.472
																	c-12.473,4.393-19.023,18.066-14.629,30.539c3.38,9.596,12.455,16.009,22.629,15.989c4.418,0,8,3.582,8,8s-3.582,8-8,8
																	s-8-3.582-8-8h-16c0.042,10.134,6.445,19.15,16,22.528V288h16v-9.472c12.473-4.393,19.023-18.066,14.63-30.539
																	C291.249,238.393,282.174,231.981,272,232z"/>';
																echo '</g>';
															echo '</g>';
															echo '<g>';
																echo '<g>';
																	echo '<path d="M336,232c-4.418,0-8-3.582-8-8s3.582-8,8-8s8,3.582,8,8h16c-0.042-10.134-6.445-19.15-16-22.528V192h-16v9.472
																	c-12.473,4.393-19.023,18.066-14.629,30.539c3.38,9.596,12.455,16.009,22.629,15.989c4.418,0,8,3.582,8,8s-3.582,8-8,8
																	s-8-3.582-8-8h-16c0.042,10.134,6.445,19.15,16,22.528V288h16v-9.472c12.473-4.393,19.023-18.066,14.63-30.539
																	C355.249,238.393,346.174,231.981,336,232z"/>';
																echo '</g>';
															echo '</g>';
															echo '<g>';
																echo '<g>';
																	echo '<path d="M136,200h-2.808c1.825-3.739,2.785-7.84,2.808-12c0.021-15.442-12.48-27.977-27.922-27.998
																	c-7.562-0.01-14.805,3.042-20.078,8.462c-10.681-11.103-28.341-11.444-39.444-0.763c-8.713,8.383-11.034,21.424-5.748,32.299H40
																	c-4.418,0-8,3.582-8,8v96c0,4.418,3.582,8,8,8h96c4.418,0,8-3.582,8-8v-96C144,203.582,140.418,200,136,200z M64,296H48v-80h16
																	V296z M56,188c0-6.627,5.373-12,12-12s12,5.373,12,12v12H68C61.373,200,56,194.627,56,188z M96,296H80v-80h16V296z M96,200v-12
																	c0-6.627,5.373-12,12-12s12,5.373,12,12c0,6.627-5.373,12-12,12H96z M128,296h-16v-80h16V296z"/>';
																echo '</g>';
															echo '</g>';
															echo '<g>';
																echo '<g>';
																	echo '<path d="M407.037,232.982c-2.156-2.156-4.622-3.979-7.317-5.406v0c11.539-12.299,10.922-31.624-1.377-43.162
																	c-0.832-0.781-1.708-1.515-2.623-2.198c8.559-14.593,3.668-33.361-10.924-41.92c-11.989-7.032-27.21-5.111-37.076,4.68
																	l-7.296,7.296c-7.922-14.938-26.454-20.625-41.392-12.702c-2.693,1.428-5.157,3.25-7.312,5.406l-7.296,7.296
																	c-7.922-14.938-26.454-20.625-41.392-12.702c-2.693,1.428-5.157,3.25-7.312,5.406l-34.752,34.744
																	c-11.958,11.955-11.96,31.34-0.005,43.298c2.152,2.152,4.613,3.972,7.301,5.398l-7.296,7.304
																	c-11.958,11.955-11.96,31.34-0.005,43.298c2.152,2.152,4.613,3.972,7.301,5.398l-7.296,7.304c-11.96,11.96-11.96,31.352,0,43.312
																	c11.96,11.96,31.352,11.96,43.312,0l2.528-2.52c10.506,13.175,29.703,15.339,42.879,4.834c0.908-0.724,1.774-1.499,2.593-2.322
																	l7.296-7.296c7.922,14.938,26.454,20.625,41.392,12.702c2.693-1.428,5.157-3.25,7.312-5.406l58.752-58.744
																	C418.99,264.325,418.992,244.94,407.037,232.982z M395.736,264.976h-0.016l-58.752,58.744c-5.709,5.713-14.968,5.717-20.681,0.009
																	c-2.704-2.702-4.242-6.354-4.287-10.177c0-0.192,0.12-1.024,0.128-1.216c0.204-3.318-1.665-6.417-4.696-7.784
																	c-1.035-0.462-2.155-0.702-3.288-0.704c-2.122,0-4.156,0.844-5.656,2.344L280.96,323.72c-2.749,2.73-6.462,4.267-10.336,4.28
																	c-6.608-0.03-12.371-4.497-14.048-10.888c-1.131-4.271-5.51-6.817-9.781-5.686c-1.366,0.362-2.612,1.079-3.611,2.078
																	l-10.216,10.216c-5.72,5.708-14.984,5.699-20.693-0.021c-2.737-2.742-4.274-6.457-4.275-10.331
																	c-0.01-3.881,1.531-7.605,4.28-10.344l17.528-17.52c3.126-3.123,3.128-8.188,0.006-11.314c-1.639-1.641-3.909-2.489-6.222-2.326
																	L222.624,272c-8.081-0.003-14.63-6.556-14.627-14.637c0.001-3.878,1.542-7.597,4.283-10.339l17.528-17.52
																	c3.102-3.146,3.066-8.211-0.08-11.313c-1.625-1.603-3.858-2.435-6.136-2.287L222.624,216c-8.081-0.003-14.63-6.556-14.627-14.637
																	c0.001-3.878,1.542-7.597,4.283-10.339l34.752-34.744c5.709-5.713,14.968-5.717,20.681-0.008
																	c2.704,2.702,4.243,6.354,4.287,10.177c0,0.192-0.12,1.024-0.128,1.216c-0.268,4.41,3.09,8.202,7.5,8.47
																	c2.285,0.139,4.52-0.708,6.14-2.326l17.528-17.528c5.713-5.709,14.972-5.706,20.681,0.007c2.697,2.699,4.232,6.345,4.279,10.161
																	c0,0.192-0.12,1.024-0.128,1.216c-0.268,4.41,3.09,8.202,7.5,8.47c2.285,0.139,4.52-0.708,6.14-2.326l17.528-17.528
																	c5.724-5.704,14.989-5.688,20.693,0.037c2.73,2.739,4.264,6.448,4.267,10.315c0.011,3.875-1.527,7.593-4.272,10.328l-2.224,2.208
																	c-3.129,3.119-3.138,8.184-0.019,11.314c1,1.004,2.249,1.724,3.619,2.086c7.785,2.03,12.451,9.986,10.422,17.772
																	c-0.655,2.512-1.969,4.803-3.806,6.636l-9.528,9.52c-3.126,3.123-3.128,8.188-0.006,11.314c1.639,1.641,3.909,2.489,6.222,2.326
																	L385.6,240c8.08,0.117,14.536,6.762,14.419,14.842C399.964,258.648,398.428,262.284,395.736,264.976z"/>';
																echo '</g>';
															echo '</g>';
														echo '</svg>';
														echo '<span class="page-games-text '.$delete_rubbellose.'">Rubbellose</span>';
													echo '</div>';
													echo '<div class="page-games-item">';
														echo '<svg class="page-games-ikon '.$delete_keno.'" id="Layer_3" enable-background="new 0 0 64 64" height="64" viewBox="0 0 64 64" width="64" xmlns="http://www.w3.org/2000/svg">';
															echo '<g>';
																echo '<path d="m15 11c1.103 0 2 .897 2 2v2h2v-2c0-1.2-.542-2.266-1.382-3 .84-.734 1.382-1.8 1.382-3v-2h-2v2c0 1.103-.897 2-2 2v-4h-2v10h2z"/>';
																echo '<path d="m22 15h4v-2h-3v-2h3v-2h-3v-2h3v-2h-4c-.552 0-1 .448-1 1v8c0 .552.448 1 1 1z"/>';
																echo '<path d="m30 9.487 3.152 5.043c.186.298.51.47.848.47.092 0 .185-.013.276-.039.429-.123.724-.515.724-.961v-9h-2v5.513l-3.152-5.043c-.236-.379-.696-.553-1.124-.432-.429.124-.724.516-.724.962v9h2z"/>';
																echo '<path d="m41 15c2.206 0 4-1.794 4-4v-2c0-2.206-1.794-4-4-4s-4 1.794-4 4v2c0 2.206 1.794 4 4 4zm-2-6c0-1.103.897-2 2-2s2 .897 2 2v2c0 1.103-.897 2-2 2s-2-.897-2-2z"/>';
																echo '<path d="m12 21h-6c-.552 0-1 .448-1 1v6c0 .552.448 1 1 1h6c.552 0 1-.448 1-1v-6c0-.552-.448-1-1-1zm-1 6h-4v-4h4z"/>';
																echo '<path d="m22 21h-6c-.552 0-1 .448-1 1v6c0 .552.448 1 1 1h6c.552 0 1-.448 1-1v-6c0-.552-.448-1-1-1zm-1 6h-4v-4h4z"/>';
																echo '<path d="m26 29h6c.552 0 1-.448 1-1v-6c0-.552-.448-1-1-1h-6c-.552 0-1 .448-1 1v6c0 .552.448 1 1 1zm1-6h4v4h-4z"/>';
																echo '<path d="m36 29h6c.552 0 1-.448 1-1v-6c0-.552-.448-1-1-1h-6c-.552 0-1 .448-1 1v6c0 .552.448 1 1 1zm1-6h4v4h-4z"/>';
																echo '<path d="m26 39h6c.552 0 1-.447 1-1v-6c0-.552-.448-1-1-1h-6c-.552 0-1 .448-1 1v6c0 .553.448 1 1 1zm1-6h4v4h-4z"/>';
																echo '<path d="m22 31h-6c-.552 0-1 .448-1 1v6c0 .553.448 1 1 1h6c.552 0 1-.447 1-1v-6c0-.552-.448-1-1-1zm-1 6h-4v-4h4z"/>';
																echo '<path d="m12 31h-6c-.552 0-1 .448-1 1v6c0 .553.448 1 1 1h6c.552 0 1-.447 1-1v-6c0-.552-.448-1-1-1zm-1 6h-4v-4h4z"/>';
																echo '<path d="m12 41h-6c-.552 0-1 .447-1 1v6c0 .553.448 1 1 1h6c.552 0 1-.447 1-1v-6c0-.553-.448-1-1-1zm-1 6h-4v-4h4z"/>';
																echo '<path d="m22 41h-6c-.552 0-1 .447-1 1v6c0 .553.448 1 1 1h6c.552 0 1-.447 1-1v-6c0-.553-.448-1-1-1zm-1 6h-4v-4h4z"/>';
																echo '<path d="m52 41h-6c-.552 0-1 .447-1 1v6c0 .553.448 1 1 1h6c.552 0 1-.447 1-1v-6c0-.553-.448-1-1-1zm-1 6h-4v-4h4z"/>';
																echo '<path d="m12 51h-6c-.552 0-1 .447-1 1v6c0 .553.448 1 1 1h6c.552 0 1-.447 1-1v-6c0-.553-.448-1-1-1zm-1 6h-4v-4h4z"/>';
																echo '<path d="m22 51h-6c-.552 0-1 .447-1 1v6c0 .553.448 1 1 1h6c.552 0 1-.447 1-1v-6c0-.553-.448-1-1-1zm-1 6h-4v-4h4z"/>';
																echo '<path d="m31 57h-4v-3.009h-2v4.009c0 .553.448 1 1 1h6c.552 0 1-.447 1-1v-6h-2z"/>';
																echo '<path d="m42 51h-6c-.552 0-1 .447-1 1v6c0 .553.448 1 1 1h6c.552 0 1-.447 1-1v-6c0-.553-.448-1-1-1zm-1 6h-4v-4h4z"/>';
																echo '<path d="m52 51h-6c-.552 0-1 .447-1 1v6c0 .553.448 1 1 1h6c.552 0 1-.447 1-1v-6c0-.553-.448-1-1-1zm-1 6h-4v-4h4z"/>';
																echo '<path d="m62.707 19.293-4-4c-.391-.391-1.023-.391-1.414 0l-.293.293v-13.586c0-.552-.448-1-1-1h-54c-.552 0-1 .448-1 
																1v60c0 .553.448 1 1 1h54c.552 0 1-.447 1-1v-35.586l5.707-5.707c.391-.391.391-1.023 0-1.414zm-7.707-16.293v14h-52v-14zm0 
																58h-52v-42h50.586l-2 2h-5.586c-.552 0-1 .448-1 1v5.586l-3.414 3.414h-5.586c-.552 0-1 .448-1 1v5.586l-3.414 3.414h-5.586c-.552 
																0-1 .447-1 1v6c0 .553.448 1 1 1h.382l-1.276 2.553c-.192.385-.117.85.188 1.154.191.192.447.293.706.293.151 0 .305-.034.447-.105l8-4c.096-.048.184-.111.26-.188l.363-.363c.141.382.5.656.93.656h6c.552 
																0 1-.447 1-1v-6c0-.43-.275-.789-.655-.931l2.725-2.725c.141.382.5.656.93.656h6c.552 0 1-.447 1-1v-6c0-.43-.275-.789-.656-.93l2.656-2.656zm-5.414-38-2.586 
																2.586v-2.586zm-10 10-2.586 2.586v-2.586zm-12.204 14h-.382v-4h2.586l-.293.293c-.076.076-.139.164-.188.26zm13.032-4h.586v4h-4v-.586zm10-10h.586v4h-4v-.586zm-17.006 
																14.178-5.172 2.586 2.586-5.171 27.178-27.179 2.586 2.586z"/>';
															echo '</g>';
														echo '</svg>';
														echo '<span class="page-games-text '.$delete_keno.'">Keno</span>';
													echo '</div>';
													echo '<div class="page-games-item">';
														echo '<svg class="page-games-ikon '.$delete_craps.'" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="mdi-dice-multiple" width="64" height="64" viewBox="0 0 24 24">';
															echo '<path d="M19.78,3H11.22C10.55,3 10,3.55 10,4.22V8H16V14H19.78C20.45,14 21,13.45 21,12.78V4.22C21,3.55 
															20.45,3 19.78,3M12.44,6.67C11.76,6.67 11.21,6.12 11.21,5.44C11.21,4.76 11.76,4.21 12.44,4.21A1.23,1.23 0 0,1 
															13.67,5.44C13.67,6.12 13.12,6.67 12.44,6.67M18.56,12.78C17.88,12.79 17.33,12.24 17.32,11.56C17.31,10.88 17.86,10.33 
															18.54,10.32C19.22,10.31 19.77,10.86 19.78,11.56C19.77,12.23 19.23,12.77 18.56,12.78M18.56,6.67C17.88,6.68 17.33,6.13 
															17.32,5.45C17.31,4.77 17.86,4.22 18.54,4.21C19.22,4.2 19.77,4.75 19.78,5.44C19.78,6.12 19.24,6.66 
															18.56,6.67M4.22,10H12.78A1.22,1.22 0 0,1 14,11.22V19.78C14,20.45 13.45,21 12.78,21H4.22C3.55,21 3,20.45 
															3,19.78V11.22C3,10.55 3.55,10 4.22,10M8.5,14.28C7.83,14.28 7.28,14.83 7.28,15.5C7.28,16.17 7.83,16.72 8.5,16.72C9.17,16.72 
															9.72,16.17 9.72,15.5A1.22,1.22 0 0,0 8.5,14.28M5.44,11.22C4.77,11.22 4.22,11.77 4.22,12.44A1.22,1.22 0 0,0 
															5.44,13.66C6.11,13.66 6.66,13.11 6.66,12.44V12.44C6.66,11.77 6.11,11.22 5.44,11.22M11.55,17.33C10.88,17.33 10.33,17.88 
															10.33,18.55C10.33,19.22 10.88,19.77 11.55,19.77A1.22,1.22 0 0,0 12.77,18.55H12.77C12.77,17.88 12.23,17.34 11.56,17.33H11.55Z" />';
														echo '</svg>';
														echo '<span class="page-games-text '.$delete_craps.'">Craps</span>';
													echo '</div>';
												echo '</div>';
											echo '</div>';
											echo '<div class="card-page-jackpots">';
												echo '<div class="card-page-games-title">';
													echo '<span>Verfügbare Jackpots</span>';
												echo '</div>';
												echo '<div class="card-page-games-content">';
													echo '<div class="page-jackpots-item">';
														echo '<div class="page-jackpots-image">';
															echo '<svg class="page-jackpots-ikon '.$delete_casino.'" id="Layer_1" enable-background="new 0 0 512 512" height="64" viewBox="0 0 512 512" width="64" xmlns="http://www.w3.org/2000/svg">';
																echo '<path d="m36.5 353.3c11.3 25.5 27 48.8 46.3 68.9l65.2-65.2c-10.9-11.6-19.8-24.9-26.4-39.3z"/>';
																echo '<path d="m256 176.1c2.9 0 5.2-2.3 5.2-5.2s-2.3-5.2-5.2-5.2-5.2 2.3-5.2 5.2 2.3 5.2 5.2 5.2z"/>';
																echo '<path d="m117.3 204.5-85.3-35c-10 26-15.4 53.6-15.9 81.5h92.1c.5-15.9 3.6-31.6 9.1-46.5z"/>';
																echo '<path d="m117.7 308.4c-5.8-15.2-9-31.2-9.5-47.4h-92.1c.5 28.4 6.1 56.6 16.5 83z"/>';
																echo '<path d="m251 403.8c-15.9-.5-31.6-3.6-46.5-9.1l-35 85.3c26 10 53.6 15.4 81.5 15.9z"/>';
																echo '<path d="m169.4 262.8c2.9 0 5.2-2.3 5.2-5.2s-2.3-5.2-5.2-5.2-5.2 2.3-5.2 5.2c0 2.8 2.3 5.1 5.2 5.2z"/>';
																echo '<path d="m342.5 32c-26-10-53.6-15.4-81.5-15.9v92.1c15.9.5 31.6 3.6 46.5 9.1z"/>';
																echo '<path d="m403.8 251h92.1c-.5-28.4-6.1-56.6-16.5-83l-85.1 35.6c5.8 15.1 9 31.2 9.5 47.4z"/>';
																echo '<path d="m356.9 148 65.2-65.2c-20.5-19.7-44.3-35.7-70.4-47l-35 85.3c14.8 6.7 28.4 15.8 40.2 26.9z"/>';
																echo '<path d="m390.4 194.3 85.1-35.6c-11.3-25.5-27-48.8-46.3-68.9l-65.2 65.3c10.9 11.6 19.8 24.8 26.4 39.2z"/>';
																echo '<path d="m148 155.1-65.2-65.2c-19.7 20.5-35.7 44.3-47 70.4l85.3 35c6.7-14.8 15.8-28.4 26.9-40.2z"/>';
																echo '<path d="m251 108.2v-92.1c-28.4.5-56.6 6.1-83 16.5l35.6 85.1c15.1-5.8 31.2-9 47.4-9.5z"/>';
																echo '<path d="m194.3 121.6-35.6-85.1c-25.5 11.3-48.8 27-68.9 46.3l65.2 65.2c11.7-10.9 24.9-19.8 39.3-26.4z"/>';
																echo '<path d="m195.3 390.9c-14.8-6.7-28.4-15.8-40.2-26.9l-65.2 65.2c20.5 19.7 44.3 35.7 70.4 47z"/>';
																echo '<circle cx="368.2" cy="422.7" r="15.4"/>';
																echo '<path d="m261 403.8v92.1c28.4-.5 56.6-6.1 83-16.5l-35.6-85.1c-15.1 5.8-31.2 9-47.4 9.5z"/>';
																echo '<path d="m422.1 429.2-65.2-65.2c-11.6 10.9-24.9 19.8-39.3 26.4l35.6 85.1c25.5-11.3 48.8-27 
																	68.9-46.3zm-79.3-6.5c0-14 11.4-25.4 25.4-25.4s25.4 11.4 25.4 25.4-11.4 25.4-25.4 25.4c-14 0-25.4-11.4-25.4-25.4z"/>';
																echo '<path d="m476.2 351.7-85.3-35c-6.7 14.8-15.8 28.4-26.9 40.2l65.2 65.2c19.7-20.5 35.7-44.3 47-70.4z"/>';
																echo '<path d="m118.1 256c0 76 61.9 137.9 137.9 137.9s137.9-61.9 137.9-137.9-61.9-137.9-137.9-137.9-137.9 61.9-137.9 
																	137.9zm51.3-13.7c6.5 0 12.2 4.1 14.3 10.2h36.8c1.6-16.4 14.2-29.5 30.5-31.8v-35.5c-7.9-2.8-12.1-11.4-9.4-19.4s11.4-12.1 
																	19.4-9.4 12.1 11.4 9.4 19.4c-1.5 4.4-5 7.8-9.4 9.4v35.5c16.3 2.3 28.8 15.5 30.5 31.8h36.8c2.8-7.9 11.4-12.1 19.4-9.4 
																	7.9 2.8 12.1 11.4 9.4 19.4s-11.4 12.1-19.4 9.4c-4.4-1.5-7.8-5-9.4-9.4h-37.3c-2.8 15-14.9 26.6-30 28.7v38.5c7.9 2.8 12.1 
																	11.4 9.4 19.4-2.8 7.9-11.4 12.1-19.4 9.4s-12.1-11.4-9.4-19.4c1.5-4.4 5-7.8 9.4-9.4v-38.5c-15.1-2.2-27.2-13.7-30-28.7h-37.3c-2.8 
																	7.9-11.4 12.1-19.4 9.3-7.9-2.8-12.1-11.4-9.3-19.4 2.2-6 7.9-10.1 14.4-10.1z"/>';
																echo '<circle cx="256" cy="344.2" r="5.2"/>';
																echo '<path d="m495.9 261h-92.1c-.5 15.9-3.6 31.6-9.1 46.5l85.3 35c10-26.1 15.4-53.6 15.9-81.5z"/>';
																echo '<circle cx="256" cy="256" r="4"/>';
																echo '<path d="m256 281.6c14.2 0 25.6-11.5 25.6-25.6 0-14.2-11.5-25.6-25.6-25.6-14.2 0-25.6 11.5-25.6 25.6s11.5 
																	25.6 25.6 25.6zm0-39.6c7.7 0 14 6.3 14 14s-6.3 14-14 14-14-6.3-14-14 6.3-14 14-14z"/>';
																echo '<path d="m342.6 262.8c2.9 0 5.2-2.3 5.2-5.2s-2.3-5.2-5.2-5.2-5.2 2.3-5.2 5.2c0 2.8 2.3 5.1 5.2 5.2z"/>';
															echo '</svg>';
															echo '<span class="page-jackpots-title '.$delete_casino.'">Casino</span>';
														echo '</div>';
														echo '<div class="page-jackpots-img-ikon '.$delete_casino.'">';
														echo '</div>';
													echo '</div>';
													echo '<div class="page-jackpots-item">';
														echo '<div class="page-jackpots-image">';
															echo '<svg class="page-jackpots-ikon '.$delete_sports.'" height="64" viewBox="0 0 480 480" width="64" xmlns="http://www.w3.org/2000/svg">';
																echo '<path d="m240 0c-132.546875 0-240 107.453125-240 240s107.453125 240 240 240 240-107.453125 
																	240-240c-.148438-132.484375-107.515625-239.851562-240-240zm8.566406 69.191406 83.433594-33.351562c9.46875 
																	4.285156 18.628906 9.222656 27.414062 14.777344l.21875.136718c8.632813 5.46875 16.882813 11.519532 24.695313 
																	18.109375l.671875.585938c3.503906 2.984375 6.910156 6.074219 10.222656 9.261719.417969.410156.855469.800781 1.273438 
																	1.21875 3.472656 3.390624 6.835937 6.886718 10.089844 10.484374.269531.304688.527343.625.796874.929688 2.855469 3.199219 
																	5.601563 6.511719 8.265626 9.878906.640624.800782 1.28125 1.601563 1.902343 2.402344 2.890625 3.742188 5.6875 7.550781 
																	8.328125 11.480469l-16.632812 70.703125-81.832032 27.28125-78.828124-63.074219zm-186.125 34.480469c.621094-.800781 
																	1.253906-1.601563 1.894532-2.398437 2.632812-3.339844 5.355468-6.597657 8.167968-9.777344.304688-.335938.585938-.679688.886719-1.015625 
																	3.234375-3.605469 6.582031-7.097657 10.050781-10.480469.398438-.390625.796875-.800781 1.214844-1.160156 3.285156-3.167969 
																	6.664062-6.238282 10.136719-9.207032l.800781-.671874c7.742188-6.542969 15.914062-12.554688 
																	24.460938-18l.3125-.199219c8.734374-5.542969 17.835937-10.472657 27.25-14.761719l83.816406 33.191406v80.800782l-78.832032 
																	63.0625-81.832031-27.230469-16.632812-70.703125c2.664062-3.921875 5.429687-7.722656 8.304687-11.449219zm-9.640625 
																	259.089844c-2.351562-3.585938-4.601562-7.238281-6.746093-10.960938l-.519532-.898437c-2.132812-3.703125-4.152344-7.46875-6.054687-11.292969l-.066407-.121094c-4.007812-8.046875-7.527343-16.328125-10.535156-24.800781v-.078125c-1.421875-4-2.71875-8.097656-3.917968-12.21875l-.433594-1.519531c-1.097656-3.871094-2.09375-7.785156-2.984375-11.742188-.078125-.386718-.175781-.753906-.253907-1.136718-1.964843-8.9375-3.375-17.984376-4.226562-27.097657l48.839844-58.605469 
																	81.265625 27.085938 23.585937 94.335938-38.753906 51.5625zm240.472657 94.78125c-4 .992187-8.105469 1.847656-12.210938 
																	2.617187-.574219.113282-1.160156.207032-1.734375.3125-3.496094.625-7.03125 1.160156-10.574219 
																	1.597656-.945312.121094-1.882812.25-2.824218.363282-3.289063.382812-6.609376.671875-9.9375.910156-1.046876.070312-2.082032.175781-3.128907.242188-4.253906.261718-8.542969.414062-12.863281.414062-3.957031 
																	0-7.890625-.105469-11.800781-.3125-.472657 0-.925781-.078125-1.398438-.113281-3.480469-.199219-6.945312-.460938-10.402343-.796875l-.398438-.074219c-7.574219-.820313-15.105469-2.023437-22.558594-3.597656l-47.320312-74.089844 
																	38.144531-50.863281h111.46875l38.769531 51.199218zm165.496093-169.542969c-.082031.382812-.175781.753906-.257812 1.136719-.894531 
																	3.953125-1.890625 7.867187-2.984375 11.742187l-.429688 1.519532c-1.203125 4.121093-2.496094 8.203124-3.921875 
																	12.21875v.078124c-3.007812 8.472657-6.523437 16.753907-10.535156 24.800782l-.066406.121094c-1.914063 3.828124-3.929688 
																	7.59375-6.054688 11.292968l-.519531.898438c-2.132812 3.734375-4.378906 7.378906-6.734375 10.945312l-78.929687 
																	12.445313-39.023438-51.519531 23.574219-94.3125 81.265625-27.085938 48.839844 58.605469c-.847657 9.117187-2.257813 
																	18.171875-4.222657 27.113281zm0 0"/>';
															echo '</svg>';
															echo '<span class="page-jackpots-title '.$delete_sports.'">Sports</span>';
														echo '</div>';
														echo '<div class="page-jackpots-img-ikon '.$delete_sports.'">';
														echo '</div>';
													echo '</div>';
													echo '<div class="page-jackpots-item">';
														echo '<div class="page-jackpots-image">';
															echo '<svg class="page-jackpots-ikon '.$delete_live.'" id="Layer_5" enable-background="new 0 0 64 64" height="64" viewBox="0 0 64 64" width="64" xmlns="http://www.w3.org/2000/svg">';
																echo '<g>';
																	echo '<circle cx="20" cy="15" r="1"/>';
																	echo '<circle cx="26" cy="15" r="1"/>';
																	echo '<path d="m26.789 19.658.105-.211-1.789-.895-.105.212c-.381.762-1.147 1.236-2 
																		1.236s-1.619-.474-2-1.236l-.105-.211-1.789.895.105.211c.722 1.444 2.174 2.341 3.789 2.341s3.067-.897 3.789-2.342z"/>';
																	echo '<path d="m50 27c7.168 0 13-5.832 13-13s-5.832-13-13-13-13 5.832-13 13 5.832 13 13 13zm0-20c3.859 0 7 3.141 7 7s-3.141 7-7 
																		7-7-3.141-7-7 3.141-7 7-7zm-5.611 14.025c1.295 1.036 2.878 1.723 4.611 1.916v2.008c-2.283-.207-4.366-1.111-6.034-2.501zm6.611 
																		1.916c1.733-.193 3.316-.88 4.611-1.916l1.423 1.423c-1.667 1.391-3.751 2.294-6.034 2.501zm6.025-3.33c1.036-1.295 1.723-2.878 
																		1.916-4.611h2.008c-.207 2.283-1.111 4.366-2.501 6.034zm1.916-6.611c-.193-1.733-.88-3.316-1.916-4.611l1.423-1.423c1.391 1.667 
																		2.294 3.751 2.501 6.034zm-3.33-6.025c-1.295-1.036-2.878-1.723-4.611-1.916v-2.008c2.283.207 4.366 1.111 
																		6.034 2.501zm-6.611-1.916c-1.733.193-3.316.88-4.611 1.916l-1.423-1.423c1.668-1.391 3.751-2.294 6.034-2.501zm-6.025 
																		3.33c-1.036 1.295-1.723 2.878-1.916 4.611h-2.008c.207-2.283 1.111-4.366 2.501-6.034zm-1.916 6.611c.193 1.733.88 3.316 
																		1.916 4.611l-1.423 1.423c-1.391-1.667-2.294-3.751-2.501-6.034z"/>';
																	echo '<path d="m54.25 14-4.25-5.667-4.25 5.667 4.25 5.667zm-4.25-2.333 1.75 2.333-1.75 2.333-1.75-2.333z"/>';
																	echo '<path d="m56 37h-8c-1.302 0-2.402.839-2.816 2h-4.184v-2.732c0-2.374-.925-4.606-2.604-6.285-1.245-1.245-2.816-2.086-4.543-2.432l-6.853-1.371v-1.262c1.828-1.061 
																		3.195-2.823 3.737-4.918h.263c2.206 0 4-1.794 4-4 0-.791-.238-1.524-.635-2.145.415-1.64.635-3.325.635-5.017v-1.559l-2.441-.813 
																		1.957-2.937-2.534-1.014c-2.514-1.005-5.162-1.515-7.87-1.515h-3.064c-2.191 0-4.314.643-6.139 1.858-2.448 1.633-3.909 
																		4.364-3.909 7.305 0 1.209.268 2.413.752 3.519-.469.656-.752 1.452-.752 2.318 0 2.206 1.794 4 4 4h.263c.542 2.095 1.909 3.856 
																		3.737 4.918v1.262l-6.854 1.371c-1.727.346-3.298 1.187-4.543 2.432-1.678 1.678-2.603 3.911-2.603 6.285v2.732h-4v24h62v-24h-4.184c-.414-1.161-1.514-2-2.816-2zm1 
																		15c0 .552-.448 1-1 1h-8c-.552 0-1-.448-1-1s.448-1 1-1h8c.552 0 1 .448 1 1zm0-8c0 .552-.448 1-1 1h-8c-.552 0-1-.448-1-1s.448-1 
																		1-1h8c.552 0 1 .448 1 1zm0 4c0 .552-.448 1-1 1h-8c-.552 0-1-.448-1-1s.448-1 1-1h8c.552 0 1 .448 1 1zm-9-9h8c.552 0 1 .448 1 
																		1s-.448 1-1 1h-8c-.552 0-1-.448-1-1s.448-1 1-1zm-17-21v-4c1.103 0 2 .897 2 2s-.897 2-2 2zm-18-2c0-1.103.897-2 2-2v4c-1.103 
																		0-2-.897-2-2zm2-6v2c-.581 0-1.13.131-1.629.355-.241-.705-.371-1.448-.371-2.192 0-2.271 1.129-4.38 3.019-5.641 1.494-.996 
																		3.233-1.522 5.029-1.522h3.064c2.452 0 4.85.462 7.126 1.373l.246.098-2.043 3.063 3.559 1.187v.117c0 1.186-.12 2.368-.346 
																		3.53-.506-.231-1.063-.368-1.654-.368v-2h-1.395c-2.16 0-4.253-.634-6.051-1.832l-.554-.37-.555.37c-1.798 1.198-3.89 1.832-6.05 
																		1.832zm2 8v-6.014c2.123-.099 4.175-.716 6-1.805 1.825 1.089 3.877 1.706 6 1.805v6.014c0 3.309-2.691 6-6 6s-6-2.691-6-6zm6 
																		8c.692 0 1.359-.097 2-.263v1.263c0 1.103-.897 2-2 2s-2-.897-2-2v-1.263c.641.166 1.308.263 2 .263zm-16 10.268c0-1.84.717-3.57 
																		2.018-4.871.965-.965 2.183-1.617 3.521-1.885l6.659-1.332c.507 1.628 2.009 2.82 3.802 2.82s3.295-1.192 3.802-2.82l6.659 
																		1.332c1.339.268 2.557.92 3.521 1.885 1.301 1.3 2.018 3.031 2.018 4.871v5.732c0 1.654-1.346 3-3 3h-3v-4c1.103 0 2-.897 
																		2-2v-4h-2v4h-4v-3c0-1.654-1.346-3-3-3h-6c-1.654 0-3 1.346-3 3v3h-4v-4h-2v4c0 1.103.897 2 2 2v4h-3c-1.654 0-3-1.346-3-3zm14 
																		4.732c1.103 0 2 .897 2 2s-.897 2-2 2h-1-5v-4zm0-2h-2v-3c0-.552.448-1 1-1h6c.552 0 1 .448 1 1v9h-2.556c.344-.591.556-1.268.556-2 0-2.206-1.794-4-4-4zm8 2h2v4h-2zm-26 20v-2h58v2zm58-4h-58v-16h2v1c0 2.757 2.243 5 5 5h10 1 15c2.757 
																		0 5-2.243 5-5v-1h4.184c.133.374.335.711.596 1-.48.532-.78 1.229-.78 2s.301 1.468.78 2c-.48.532-.78 1.229-.78 2s.301 1.468.78 2c-.48.532-.78 1.229-.78 2 0 1.654 1.346 3 3 3h8c1.654 0 3-1.346 3-3 0-.771-.301-1.468-.78-2 
																		.48-.532.78-1.229.78-2s-.301-1.468-.78-2c.48-.532.78-1.229.78-2s-.301-1.468-.78-2c.26-.289.463-.626.596-1h2.184z"/>';
																echo '</g>';
															echo '</svg>';
															echo '<span class="page-jackpots-title '.$delete_live.'">Live casino</span>';
														echo '</div>';
														echo '<div class="page-jackpots-img-ikon '.$delete_live.'">';
														echo '</div>';
													echo '</div>';
													echo '<div class="page-jackpots-item">';
														echo '<div class="page-jackpots-image">';
															echo '<svg class="page-jackpots-ikon '.$delete_pok.'" id="Layer_1" height="64" viewBox="0 0 512 512" width="64" xmlns="http://www.w3.org/2000/svg" data-name="Layer 1">';
																echo '<path d="m127.205 97.001h17.546l-8.773-30.959z"/>';
																echo '<path d="m243.823 357.214a66.7 66.7 0 0 1 24.354 0l-12.177-28.153z"/>';
																echo '<path d="m409.125 5h-306.25a36.667 36.667 0 0 0 -36.625 36.625v428.75a36.667 36.667 0 0 0 36.625 36.625h306.25a36.667 36.667 0 0 0 36.625-36.625v-428.75a36.667 36.667 0 0 0 -36.625-36.625zm-284.643 102.286-3.832 13.414c-.5 
																	1.917-2.219 2.623-4.437 2.623-3.53 0-8.27-2.219-8.27-5.446a5.781 5.781 0 0 1 .2-1.008l19.463-63.431c.907-3.025 4.639-4.436 8.37-4.436 3.832 0 7.563 1.411 8.47 4.436l19.463 63.431a4.431 4.431 0 0 1 .2 1.008c0 
																	3.126-4.74 5.446-8.269 5.446-2.219 0-3.933-.706-4.438-2.623l-3.831-13.412zm218.6 208.245a64.66 64.66 0 0 1 -74.621 12.105l15.776 36.473a6 6 0 0 1 -7.735 7.953 54.95 54.95 0 0 0 -41 0 6 6 0 0 1 -7.735-7.953l15.433-35.681a64.623 
																	64.623 0 0 1 -74.27-103.658l82.747-83.485a6 6 0 0 1 4.248-1.776h.014a6 6 0 0 1 4.242 1.757l82.9 82.889a64.695 64.695 0 0 1 -.002 91.376z"/>';
																echo '<path d="m255.957 154.013-78.525 79.224a52.612 52.612 0 1 0 74.385 74.425 6 6 0 0 1 4.483-1.753l.538.022a6 6 0 0 1 3.939 1.69 52.61 52.61 0 0 0 73.815-74.98z"/>';
															echo '</svg>';
															echo '<span class="page-jackpots-title '.$delete_pok.'">Poker</span>';
														echo '</div>';
														echo '<div class="page-jackpots-img-ikon '.$delete_pok.'">';
														echo '</div>';
													echo '</div>';
													echo '<div class="page-jackpots-item">';
														echo '<div class="page-jackpots-image">';
															echo '<svg class="page-jackpots-ikon '.$delete_bookmaker.'" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="64" height="64" viewBox="0 0 511.999 511.999" style="enable-background:new 0 0 511.999 511.999;" xml:space="preserve">';
																echo '<g>';
																	echo '<g>';
																		echo '<path d="M438.09,273.32h-39.596c4.036,11.05,6.241,22.975,6.241,35.404v149.65c0,5.182-0.902,10.156-2.543,14.782h65.461
																			c24.453,0,44.346-19.894,44.346-44.346v-81.581C512,306.476,478.844,273.32,438.09,273.32z"/>';
																		echo '</g>';
																	echo '</g>';
																echo '<g>';
																	echo '<g>';
																		echo '<path d="M107.265,308.725c0-12.43,2.205-24.354,6.241-35.404H73.91c-40.754,0-73.91,33.156-73.91,73.91v81.581
																			c0,24.452,19.893,44.346,44.346,44.346h65.462c-1.641-4.628-2.543-9.601-2.543-14.783V308.725z"/>';
																	echo '</g>';
																echo '</g>';
																echo '<g>';
																	echo '<g>';
																		echo '<path d="M301.261,234.815h-90.522c-40.754,0-73.91,33.156-73.91,73.91v149.65c0,8.163,6.618,14.782,14.782,14.782h208.778
																			c8.164,0,14.782-6.618,14.782-14.782v-149.65C375.171,267.971,342.015,234.815,301.261,234.815z"/>';
																	echo '</g>';
																echo '</g>';
																echo '<g>';
																	echo '<g>';
																		echo '<path d="M256,38.84c-49.012,0-88.886,39.874-88.886,88.887c0,33.245,18.349,62.28,45.447,77.524
																			c12.853,7.23,27.671,11.362,43.439,11.362c15.768,0,30.586-4.132,43.439-11.362c27.099-15.244,45.447-44.28,45.447-77.524
																			C344.886,78.715,305.012,38.84,256,38.84z"/>';
																	echo '</g>';
																echo '</g>';
																echo '<g>';
																	echo '<g>';
																		echo '<path d="M99.918,121.689c-36.655,0-66.475,29.82-66.475,66.475c0,36.655,29.82,66.475,66.475,66.475
																			c9.298,0,18.152-1.926,26.195-5.388c13.906-5.987,25.372-16.585,32.467-29.86c4.98-9.317,7.813-19.946,7.813-31.227
																			C166.393,151.51,136.573,121.689,99.918,121.689z"/>';
																	echo '</g>';
																echo '</g>';
																echo '<g>';
																	echo '<g>';
																		echo '<path d="M412.082,121.689c-36.655,0-66.475,29.82-66.475,66.475c0,11.282,2.833,21.911,7.813,31.227
																			c7.095,13.276,18.561,23.874,32.467,29.86c8.043,3.462,16.897,5.388,26.195,5.388c36.655,0,66.475-29.82,66.475-66.475
																			C478.557,151.509,448.737,121.689,412.082,121.689z"/>';
																	echo '</g>';
																echo '</g>';
															echo '</svg>';
															echo '<span class="page-jackpots-title '.$delete_bookmaker.'">Bookmaker</span>';
														echo '</div>';
														echo '<div class="page-jackpots-img-ikon '.$delete_bookmaker.'">';
														echo '</div>';
													echo '</div>';
													echo '<div class="page-jackpots-item">';
														echo '<div class="page-jackpots-image">';
															echo '<svg class="page-jackpots-ikon '.$delete_lottery.'" height="64" viewBox="0 0 512 512" width="64" xmlns="http://www.w3.org/2000/svg">';
																echo '<path d="m511.394531 194.089844c-.375-5.507813-5.144531-9.671875-10.65625-9.296875s-9.671875 5.144531-9.296875 10.65625c.371094 5.464843.558594 10.933593.558594 16.25 0 47.1875-13.875 92.726562-40.128906 131.695312-3.082032 
																	4.582031-1.871094 10.792969 2.707031 13.878907 1.714844 1.15625 3.65625 1.707031 5.578125 1.707031 3.214844 0 6.371094-1.546875 8.304688-4.414063 28.484374-42.28125 43.539062-91.683594 43.539062-142.867187 
																	0-5.765625-.203125-11.691407-.605469-17.609375zm0 0"/>';
																echo '<path d="m20 211.699219c0-52 16.59375-101.320313 47.980469-142.632813 3.34375-4.394531 2.484375-10.671875-1.910157-14.011718-4.398437-3.339844-10.671874-2.484376-14.011718 1.914062-34.058594 44.820312-52.058594 
																	98.324219-52.058594 154.730469 0 36.144531 7.398438 71.109375 21.992188 103.917969 1.65625 3.726562 5.3125 5.9375 9.144531 5.9375 1.355469 0 2.734375-.277344 4.058593-.867188 5.042969-2.242188 7.316407-8.152344 
																	5.070313-13.199219-13.449219-30.234375-20.265625-62.460937-20.265625-95.789062zm0 0"/>';
																echo '<path d="m181.019531 107.042969c1.609375 3.929687 5.742188 6.476562 9.972657 6.148437 4.347656-.339844 8.058593-3.519531 9.015624-7.78125 1.957032-8.703125-8.171874-15.523437-15.5-10.363281-3.765624 2.652344-5.265624 
																	7.734375-3.488281 11.996094zm0 0"/>';
																echo '<path d="m323.230469 99.402344c-2.066407-5.035156-8.042969-7.503906-13.0625-5.410156-5.03125 2.101562-7.492188 8.003906-5.410157 13.050781 2.082032 5.035156 8.03125 7.503906 13.0625 5.410156 
																	5.023438-2.089844 7.507813-8.015625 5.410157-13.050781zm0 0"/>';
																echo '<path d="m184.710938 328.683594c7.550781 5.03125 17.597656-2.261719 15.179687-10.976563-1.140625-4.109375-4.882813-7.113281-9.144531-7.324219-4.253906-.210937-8.289063 2.421876-9.816406 6.390626-1.648438 4.285156-.085938 
																	9.378906 3.78125 11.910156zm0 0"/>';
																echo '<path d="m323.230469 316.542969c-1.640625-3.933594-5.714844-6.460938-9.972657-6.144531-4.351562.324218-8.046874 3.53125-9.015624 7.777343-1.992188 8.742188 8.183593 15.480469 15.507812 10.367188 3.804688-2.65625 5.230469-7.726563 
																	3.480469-12zm0 0"/>';
																echo '<path d="m386.890625 207.691406c-2.097656-5.03125-8.019531-7.492187-13.0625-5.410156-5.03125 2.082031-7.5 8.03125-5.40625 13.0625 2.089844 5.03125 8.019531 7.496094 13.058594 5.410156 5.023437-2.082031 7.503906-8.035156 
																	5.410156-13.0625zm0 0"/>';
																echo '<path d="m122.921875 219.832031c7.546875 5.03125 17.613281-2.265625 15.183594-10.984375-1.140625-4.101562-4.878907-7.105468-9.140625-7.316406-4.269532-.214844-8.273438 2.414062-9.8125 6.390625-1.660156 4.28125-.097656 9.378906 
																	3.769531 11.910156zm0 0"/>';
																echo '<path d="m390.351562 374.890625c8.585938-7.066406 16.652344-14.835937 24.109376-23.265625 34.210937-38.667969 53.050781-88.425781 53.050781-140.109375 0-116.628906-94.882813-211.515625-211.511719-211.515625s-211.511719 
																	94.886719-211.511719 211.511719c0 50.976562 18.386719 100.21875 51.773438 138.65625 7.378906 8.492187 15.382812 16.335937 23.910156 23.480469l-58.691406 123.15625c-1.476563 3.097656-1.261719 6.734374.570312 9.636718 1.832031 
																	2.90625 5.023438 4.664063 8.457031 4.664063h41.90625c2.929688 0 5.707032-1.292969 7.609376-3.523438 1.898437-2.226562 2.722656-5.1875 2.261718-8.082031-.8125-5.09375-5.371094-8.65625-10.386718-8.394531h-25.550782l50.085938-105.09375c.351562.242187.710937.46875 
																	1.066406.710937 17.472656 11.832032 36.726562 21.050782 56.921875 27.183594 5.898437 1.792969 11.878906 3.328125 17.914063 4.597656.371093.078125.738281.164063 1.109374.242188l-6.519531 24.585937h-23.234375c-5.523437 0-10 
																	4.476563-10 10 0 5.523438 4.476563 10 10 10h140.152344c5.523438 0 10-4.476562 10-10 0-5.523437-4.476562-10-10-10h-22.921875l-6.207031-23.839843c.351562-.0625.699218-.140626 1.050781-.207032 21.464844-4.089844 42.265625-11.550781 
																	61.449219-22.015625 5.386718-2.9375 10.640625-6.121093 15.753906-9.515625.332031-.222656.671875-.433594 1.003906-.65625l48.375 104.007813h-219.121094c-.164062-.007813-.328124-.011719-.496093-.011719-5.523438 0-10 4.488281-10 
																	10.011719 0 5.523437 4.476562 10 10 10h235.296875c3.417968 0 6.597656-1.742188 8.433594-4.625 1.835937-2.878907 2.074218-6.496094.636718-9.59375zm-278.988281-37.839844c-30.230469-34.796875-46.875-79.378906-46.875-125.539062 
																	0-105.597657 85.910157-191.511719 191.511719-191.511719s191.511719 85.914062 191.511719 191.511719c0 46.800781-17.058594 91.851562-48.03125 126.859375-8.550781 9.664062-17.988281 18.367187-28.136719 26.035156-17.949219 
																	13.5625-38.292969 23.929688-59.828125 30.441406-6.082031 1.839844-12.253906 3.386719-18.488281 4.605469-1.121094.21875-2.246094.410156-3.367188.609375l-4.515625-17.351562c49.574219-8.992188 93.460938-39.65625 
																	118.9375-83.691407.023438-.042969.046875-.09375.070313-.136719 15.105468-26.15625 23.089844-56.015624 23.089844-86.363281 0-30.40625-8.011719-60.320312-23.167969-86.511719 
																	0-.003906-.003907-.007812-.007813-.011718-14.933594-25.804688-36.402344-47.449219-62.089844-62.589844-26.480468-15.609375-56.808593-23.859375-87.707031-23.859375-30.898437 0-61.226562 8.25-87.703125 23.859375-25.695312 
																	15.140625-47.167968 36.789062-62.101562 62.597656 0 .003906-.003906.003906-.007813.007813-15.144531 26.191406-23.152343 56.105469-23.152343 86.507812 0 30.394531 8.007812 60.304688 23.152343 
																	86.496094.007813.015625.019531.023437.023438.039063 25.410156 43.882812 69.140625 74.507812 118.539062 83.585937l-4.429687 16.714844c-.902344-.179688-1.800782-.347657-2.695313-.539063-20.265625-4.324218-39.863281-11.980468-57.65625-22.605468-5.066406-3.023438-10.003906-6.257813-14.777343-9.726563-1.40625-1.019531-2.792969-2.066406-4.167969-3.125-10.097657-7.765625-19.464844-16.5625-27.929688-26.308594zm116.257813 
																	106.28125 26.5625-100.152343 26.074218 100.152343zm36.652344-140.796875v-29.625c14.585937-2.433594 27.882812-10.113281 37.296874-21.53125l79.902344 46.125c-23.328125 34.808594-60.207031 58.761719-101.378906 
																	65.804688zm127.210937-22.34375-79.90625-46.128906c2.570313-6.851562 3.90625-14.15625 3.90625-21.542969s-1.335937-14.691406-3.90625-21.542969l79.90625-46.136718c10.332031 20.941406 15.757813 44.140625 15.757813 67.679687 0 
																	23.53125-5.425782 46.726563-15.757813 67.671875zm-89.921875-106.53125c-9.410156-11.417968-22.710938-19.097656-37.292969-21.527344v-92.261718c23.839844 1.554687 47.007813 8.652344 67.550781 20.761718 19.851563 11.703126 36.855469 
																	27.792969 49.648438 46.894532zm-11.589844 18.253906c4.914063 7.78125 6.417969 17.996094 5.019532 26.976563-1.355469 8.695313-5.53125 16.832031-11.8125 22.992187-11.652344 11.433594-29.40625 14.972657-44.519532 
																	8.765626-16.296875-6.699219-26.773437-23.472657-25.503906-41.085938 1.238281-17.175781 13.429688-32.046875 29.984375-36.714844 15.796875-4.460937 33.175781 1.214844 43.398437 14.039063 1.269532 1.585937 2.414063 3.269531 
																	3.433594 5.027343zm-45.703125-39.78125c-14.585937 2.433594-27.886719 10.109376-37.296875 21.53125l-79.902344-46.136718c12.789063-19.101563 29.796876-35.191406 49.652344-46.894532 20.542969-12.105468 43.710938-19.203124 
																	67.546875-20.761718zm-127.210937-7.292968 79.902344 46.136718c-2.574219 6.851563-3.910157 14.15625-3.910157 21.542969 0 7.382813 1.335938 14.6875 3.910157 21.539063l-79.902344 
																	46.128906c-10.332032-20.945312-15.753906-44.140625-15.753906-67.667969 0-23.535156 5.421874-46.734375 15.753906-67.679687zm89.917968 106.539062c9.410157 11.417969 22.710938 19.097656 37.296876 21.53125v29.578125l-16.113282 
																	60.769531c-41.046875-7.105468-77.816406-31.039062-101.085937-65.753906zm0 0"/>';
																echo '<path d="m163.359375 492.792969c-3.554687-2.367188-8.328125-2.183594-11.699219.433593-3.300781 2.554688-4.640625 7.0625-3.335937 11.015626 2.929687 8.847656 15.738281 8.96875 18.894531.230468 
																	1.527344-4.234375-.074219-9.207031-3.859375-11.679687zm0 0"/>';
															echo '</svg>';
															echo '<span class="page-jackpots-title '.$delete_lottery.'">Lottery</span>';
														echo '</div>';
														echo '<div class="page-jackpots-img-ikon '.$delete_lottery.'">';
														echo '</div>';
													echo '</div>';
													echo '<div class="page-jackpots-item">';
														echo '<div class="page-jackpots-image">';
															echo '<svg class="page-jackpots-ikon '.$delete_mobile.'" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 503.604 503.604" width="64" height="64" style="enable-background:new 0 0 503.604 503.604;" xml:space="preserve">';
																echo '<g>';
																	echo '<g>';
																		echo '<path d="M337.324,0H167.192c-28.924,0-53.5,23.584-53.5,52.5v398.664c0,28.916,24.056,52.44,52.98,52.44l170.412-0.184
																			c28.92,0,52.58-23.528,52.58-52.448l0.248-398.5C389.908,23.452,366.364,0,337.324,0z M227.68,31.476h49.36
																			c4.336,0,7.868,3.52,7.868,7.868c0,4.348-3.532,7.868-7.868,7.868h-49.36c-4.348,0-7.868-3.52-7.868-7.868
																			C219.812,34.996,223.332,31.476,227.68,31.476z M198.02,33.98c2.916-2.912,8.224-2.952,11.136,0c1.46,1.456,2.324,3.5,2.324,5.588
																			c0,2.048-0.864,4.088-2.324,5.548c-1.452,1.46-3.504,2.32-5.548,2.32c-2.084,0-4.088-0.86-5.588-2.32
																			c-1.452-1.456-2.28-3.5-2.28-5.548C195.736,37.48,196.568,35.436,198.02,33.98z M250.772,488.008
																			c-12.984,0-23.544-10.568-23.544-23.548c0-12.984,10.56-23.548,23.544-23.548s23.544,10.564,23.544,23.548
																			C274.316,477.44,263.752,488.008,250.772,488.008z M365.488,424.908H141.232V74.756h224.256V424.908z"/>';
																	echo '</g>';
																echo '</g>';
															echo '</svg>';
															echo '<span class="page-jackpots-title">Mobile</span>';
														echo '</div>';
														echo '<div class="page-jackpots-img-ikon '.$delete_mobile.'">';
														echo '</div>';
													echo '</div>';
												echo '</div>';
											echo '</div>';
										echo '</div>';
									echo '</div>';
								echo '</div>';
							}
						echo '</div>';
					}
				?>
			</div>
		</div>
	</div>
</section>
<section class="main-block-slots">
	<div class="head-row">
		<div class="main-slots-content">
			<div class="main-slots-title">
				<h2 class="first-text-h2 slots">Slots können Sie kostenlos spielen</h2>
			</div>
			<div class="main-slots-info">
				<?php if ($items = get_posts(array('numberposts' => 15, 'post_type' => 'Slots'))) {
					echo '<div id="owl-slider" class="owl-carousel owl-theme">';
						foreach($items as $item) {
							echo '<div class="main-slots-item">';
								echo '<a class="main-slots-item-link" href="'.get_permalink($item->ID).'">';
									echo '<div class="main-slots-block-image">';
										echo get_the_post_thumbnail($item->ID);
										echo '<div class="main-slots-block-back">';
											echo '<span class="play-btn">';
												echo '<div class="btn-ikon" style="background-image: url('.get_template_directory_uri().'/img/play.svg);"></div>';
											echo '</span>';
											echo '<div class="play-text">Kostenlos spielen</div>';
										echo '</div>';	
									echo '</div>';
									echo '<div class="main-slots-block-title">';
										echo '<span class="title-main-slots-span">'.$item->post_title.'</span>';
									echo '</div>';
								echo '</a>';
							echo '</div>';
						}
					echo '</div>';
				} ?>
			</div>
			<div class="after-cards-button">
				<a href="/online-casino-bewertungen/" class="flex-btn slots">
					<span>Alle Slots</span>
					<div class="btn-ikon all" style="background-image: url(<?php echo get_template_directory_uri() ?>/img/all-slots.svg);"></div>
				</a>
			</div>
		</div>
	</div>
</section>
<section class="main-block-bonuses">
	<div class="head-row">
		<div class="main-bonuses-content">
			<div class="main-bonuses-title">
				<h2 class="first-text-h2 bonuses">Boni für Spieler</h2>
			</div>
			<div class="main-bonuses-info">
				<div class="main-bonuses-column">
					<div class="main-bonuses-column-title">
						<div class="ikon-main-bonus" style="background-image: url(<?php echo get_template_directory_uri() ?>/img/fire.svg);"></div>
						<h3>Die top empfohlenen Boni</h3>
					</div>
					<div class="main-bonuses-column-content">
						<?php if ($items = get_posts(array('numberposts' => 10, 'post_type' => 'Bonuses', 'category' => 236))) {
							foreach($items as $item) {
								$link_brend = get_field('link_brend', $item->ID);
								$hover_text = get_field('hover_text', $item->ID);
								echo '<div class="main-bonuses-item">';
									echo '<div class="main-bonuses-item-head">';
										echo '<a href="'.get_permalink($item->ID).'">';
											echo '<div class="main-bonuses-item-image">';
												echo get_the_post_thumbnail($item->ID);
											echo '</div>';
											echo '<div class="main-bonuses-item-text left">';
												echo '<span class="main-bonuses-item-title">'.$item->post_title.'</span>';
											echo '</div>';
										echo '</a>';
									echo '</div>';
									echo '<div class="main-bonuses-item-right">';
										echo '<div class="main-bonuses-item-buttons">';
											echo '<a href="'.$link_brend.'" class="flex-btn bonus">';
												echo '<span>Erhalten bonus</span>';
												echo '<svg version="1.1" id="Capa_1" class="btn-ikon-bonus" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
													viewBox="0 0 32 32" style="enable-background:new 0 0 32 32;" xml:space="preserve">';
													echo '<g>';
														echo '<g id="arrow_x5F_right_x5F_alt1">';
															echo '<path d="M16,0C7.164,0,0,7.164,0,16s7.164,16,16,16s16-7.164,16-16S24.836,0,16,0z M16.031,24v-6H8v-4
																h8.031V8.031L24,16L16.031,24z"/>';
														echo '</g>';
													echo '</g>';
												echo '</svg>';
											echo '</a>';
											echo '<div class="tc-apply">';
												echo '<div class="tc-apply-title">';
													echo '<div class="tc-apply-ikon" style="background-image: url('.get_template_directory_uri().'/img/question.svg);"></div>';
													echo '<span class="tc-apply-text">T&C’s Apply</span>';
												echo '</div>';
												echo '<div id="tcapply">'.$hover_text.'</div>';
											echo '</div>';
										echo '</div>';
									echo '</div>';
								echo '</div>';
							}
						} ?>
					</div>
					<div class="after-cards-button">
						<a href="/online-casino-bewertungen/" class="flex-btn all">
							<span>Alle Boni</span>
							<div class="btn-ikon all" style="background-image: url(<?php echo get_template_directory_uri() ?>/img/all.svg);"></div>
						</a>
					</div>
				</div>
				<div class="main-bonuses-column">
					<div class="main-bonuses-column-title">
						<div class="ikon-main-bonus" style="background-image: url(<?php echo get_template_directory_uri() ?>/img/star.svg);"></div>
						<h3>Die aktuellen Boni</h3>
					</div>
					<div class="main-bonuses-column-content">
						<?php if ($items = get_posts(array('numberposts' => 10, 'post_type' => 'Bonuses', 'category' => 235))) {
							foreach($items as $item) {
								$link_brend = get_field('link_brend', $item->ID);
								$hover_text = get_field('hover_text', $item->ID);
								echo '<div class="main-bonuses-item">';
									echo '<div class="main-bonuses-item-head">';
										echo '<a href="'.get_permalink($item->ID).'">';
											echo '<div class="main-bonuses-item-image">';
												echo get_the_post_thumbnail($item->ID);
											echo '</div>';
											echo '<div class="main-bonuses-item-text">';
												echo '<span class="main-bonuses-item-title">'.$item->post_title.'</span>';
											echo '</div>';
										echo '</a>';
									echo '</div>';
									echo '<div class="main-bonuses-item-right">';
										echo '<div class="bonuses-block-date">';
											echo '<span class="bonus-date-text">'.get_the_date('Y-m-d', $item->ID).'</span>';
											echo '<div class="tc-apply">';
												echo '<div class="tc-apply-title">';
													echo '<div class="tc-apply-ikon" style="background-image: url('.get_template_directory_uri().'/img/question.svg);"></div>';
													echo '<span class="tc-apply-text">T&C’s Apply</span>';
												echo '</div>';
												echo '<div id="tcapply">'.$hover_text.'</div>';
											echo '</div>';
								  		echo '</div>';
									echo '</div>';
								echo '</div>';
							}
						} ?>
					</div>
					<div class="after-cards-button">
						<a href="/online-casino-bewertungen/" class="flex-btn all">
							<span>Alle Boni</span>
							<div class="btn-ikon all" style="background-image: url(<?php echo get_template_directory_uri() ?>/img/all.svg);"></div>
						</a>
					</div>
				</div>
			</div>
		</div>
	</div>
</section>
<section class="page-providers-text">
	<div class="head-row">
		<div class="page-block-content text">
			<div class="page-text-content">
				<div class="second-text">
					<?php the_content(); ?>
				</div>
				<div class="more-details-block open-btn">
					<div class="detail-btn">
						<div class="more-detail-ikon" style="background-image: url(<?php echo get_template_directory_uri() ?>/img/down.svg);"></div>
						<span>Mehr details</span>
					</div>
				</div>
				<div class="more-details-block close-btn">
					<div class="detail-btn">
						<div class="more-detail-ikon close" style="background-image: url(<?php echo get_template_directory_uri() ?>/img/down.svg);"></div>
						<span>Ausblenden</span>
					</div>
				</div>
			</div>
		</div>
	</div>
</section>

<?php get_footer()?>